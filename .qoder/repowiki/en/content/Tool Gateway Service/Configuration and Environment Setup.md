...
## Update Summary
**Changes Made**
- Updated browser tool tier classifications to reflect ad-hoc write operations with per-action approval instead of flow-bound requirements
- Enhanced security controls documentation for browser connector with updated flow-based approval enforcement mechanisms
- Revised configuration examples to support the new per-action approval model for browser write operations
- Updated troubleshooting guidance for flow-based approval scenarios and TTL expiration handling
...
HH["Per-Action Approval"]
end
subgraph "Tool Gateway Risk Control"
II["tool-gateway config.py"]
JJ["registry.py"]
KK["GATEWAY_MUTATING_TOOLS_ENABLED"]
LL["Risk-Tier Admission Gate"]
MM["Policy Enforcement"]
NN["Browser Connector"]
OO["GATEWAY_BROWSER_*"]
PP["Chromium Sidecar"]
QQ["CDP Endpoint"]
RR["Session Pool"]
SS["Credential Sets"]
TT["NetworkPolicy Enforcement"]
UU["Enumeration Prevention"]
VV["Screenshot Security"]
WW["File Upload Security"]
XX["GATEWAY_BROWSER_UPLOAD_DIR"]
YY["Ad-Hoc Write Operations"]
ZZ["Per-Action Approval Flow"]
end
subgraph "Workspace Resource Integration"
AAA["tool_gateway_url"]
BBB["skills_hub_url"]
CCC["skills_client_id"]
DDD["skills_client_secret"]
EEE["Delegated Token Flow"]
FFF["Basic Auth Flow"]
GGG["Incident Service"]
HHH["PLATFORM_GATEWAY_INCIDENT_*"]
end
subgraph "Enhanced OTel Secret Management"
III["sync-otel-secrets.sh"]
JJJ["OTEL_EXPORTER_OTLP_HEADERS"]
KKK["Cluster Secret Merge"]
LLL["Local File Preservation"]
MMM["Sibling Script Hooks"]
end
subgraph "Mutating Dev Profile"
NNN["dev-k8s/kustomization.yaml"]
OOO["runtime-profiles/mutating-dev/"]
PPP["mutating.env"]
QQQ["tool-gateway-pod-delete.yaml"]
RRR["RBAC Role/RoleBinding"]
SSS["ConfigMap Merge"]
end
subgraph "Browser Development Profile"
TTT["runtime-profiles/browser-dev/"]
UUU["browser.env"]
VVV["tool-gateway-browser-sidecar.yaml"]
WWW["chromedp/headless-shell"]
XXX["CDP ws://localhost:9222"]
YYY["Credential Sets Secret"]
ZZZ["Allow Origins"]
AAA2["NetworkPolicy"]
BBB2["Flow Approval TTL"]
CCC2["AGENT_BROWSER_FLOW_APPROVAL_TTL"]
DDD2["Per-Action Approval"]
end
subgraph "Execution Signing & Audit"
EEE2["sync-execution-signing-secret.sh"]
FFF2["execution-signing-secret"]
GGG2["HMAC-SHA256 Signing"]
HHH2["Durable Audit Trail"]
III2["Fire-and-Forget Emission"]
JJJ2["Postgres Persistence"]
KKK2["Retention Scanning"]
end
subgraph "OIDC Configuration"
LLL2["Identity Broker"]
MMM2["OIDC_REDIRECT_URI"]
NNN2["OIDC_EXTRA_REDIRECT_URIS"]
OOO2["Keycloak Client"]
PPP2["Canonical Callback"]
QQQ2["Reachability Only"]
end
A --> FFF
B --> KKK
C --> FFF
...
- **Updated**: Browser tool tier classifications now support ad-hoc write operations with per-action approval instead of flow-bound requirements, enabling more flexible browser automation workflows while maintaining security through proper authorization checks.
...
- **Updated**: Ad-hoc write operations with per-action approval: browser tools now support flexible write operations that require individual approval for each action rather than flow-bound approvals, enabling more dynamic browser automation workflows.
...
Note over Browser,FlowApprovals : Per-Action Approval Flow
...
Browser->>FlowApprovals : Check Per-Action Approval
FlowApprovals->>FlowApprovals : Validate Per-Action Authorization
FlowApprovals-->>Browser : Per-Action Approved/Rejected
...
FlowApproval --> PerActionApproval["Configure Per-Action Approval"]
PerActionApproval --> Ready["Browser Connector Ready"]
```
...
Security features include flow identity verification, TTL-based expiration monitoring, session-scoped authority management, and safe failure modes where expired approvals cause re-parking rather than unauthorized execution. The implementation uses process-wide stores for flow contexts and approvals, with atomic operations for authority recording and lookup. **Updated**: Browser tool tier classifications now support ad-hoc write operations with per-action approval instead of flow-bound requirements, enabling more flexible browser automation workflows while maintaining security through proper authorization checks and flow-based approval enforcement.
...
- **Critical Warning**: Port-forwarding scenarios cannot complete sign-in flows because the identity broker always resolves to the canonical callback URI, not the forwarded origin.

**Updated** Enhanced OIDC configuration implementation with clear behavioral distinction between canonical callback URIs and reachability-only extra URIs, including explicit warnings about port-forwarding limitations.
...
BrowserConn --> NetPol["NetworkPolicy"]
BrowserConn --> UploadDir["Upload Directory"]
NetPol --> Sidecar
...
- **Updated**: Ad-hoc write operations with per-action approval maintain performance through efficient authorization checks and flow-based approval enforcement.
...
- **New**: File upload path traversal detected: Check GATEWAY_BROWSER_UPLOAD_DIR configuration; verify uploaded filenames don't contain path separators or traversal sequences.
- **New**: File upload not configured: Ensure GATEWAY_BROWSER_UPLOAD_DIR is set to a valid directory path; verify directory exists and has proper permissions.
- **New**: File upload path escapes upload directory: Check that uploaded files are within the configured upload directory; verify path validation is working correctly.
- **New**: File upload file not found: Verify that uploaded files exist in the configured upload directory; check file permissions and accessibility.
...
- **New**: Secure GATEWAY_BROWSER_UPLOAD_DIR configuration with appropriate directory permissions and path validation.
- **New**: Monitor file upload operations for suspicious patterns and potential directory traversal attacks.
...
- **Critical Warning**: Port-forwarding cannot complete sign-in flows because the identity broker always resolves to the canonical callback URI, not the forwarded origin
...
- `GATEWAY_BROWSER_UPLOAD_DIR=/tmp/browser-uploads`: Directory for secure file uploads with path validation
...
- **File upload path validation**: Prevents directory traversal attacks through upload directory restriction

#### Expanded Tool Surface
The browser connector now supports fifteen tools total:
...
- **Write tier**: web.click, web.type, web.select, web.press_key, web.upload_file, web.evaluate (requires HITL approval)
...
data:
  ...
  GATEWAY_BROWSER_UPLOAD_DIR: "/tmp/browser-uploads"
---
apiVersion: v1
kind: Secret
metadata:
...
- Monitor file upload operations and path validation failures
- Track expanded tool surface usage and tier classification
...
- **Critical Limitation**: Port-forwarding cannot complete sign-in flows because the identity broker always resolves to the canonical callback URI, not the forwarded origin
...
- **Port-Forwarding Limitation**: Login from port-forwarded URL will redirect to canonical hostname, leaving port-forwarded tab signed out
...
- **Port-Forwarding Issues**: Understand that port-forwarded URLs cannot complete sign-in flows due to canonical callback URI enforcement
...
- **Port-Forwarding Awareness**: Recognize that port-forwarding scenarios cannot complete sign-in flows due to canonical URI enforcement
...
**New Section** Comprehensive guide for configuring and managing secure file upload operations through GATEWAY_BROWSER_UPLOAD_DIR configuration.

#### Prerequisites
- Browser connector enabled with GATEWAY_BROWSER_ENABLED=true
- Upload directory created with appropriate permissions
- Network connectivity to Chromium sidecar for file upload operations
- Understanding of file upload security implications and path validation

#### Configuration Options
- `GATEWAY_BROWSER_UPLOAD_DIR=/tmp/browser-uploads`: Directory for secure file uploads with path validation
- Default directory: `/tmp/browser-uploads` if not specified
- Path validation: Prevents directory traversal attacks through realpath resolution
- Security controls: Validates uploaded filenames don't contain path separators or traversal sequences

#### Security Features
- **Path Validation**: Realpath resolution ensures files stay within configured upload directory
- **Traversal Prevention**: Blocks filenames containing "/", "\", or ".." sequences
- **Directory Isolation**: Files are confined to the configured upload directory
- **Permission Controls**: Requires proper filesystem permissions for upload directory access
- **Logging**: Comprehensive logging of upload attempts and validation failures
...
data:
  GATEWAY_BROWSER_ENABLED: "true"
  GATEWAY_BROWSER_UPLOAD_DIR: "/secure/uploads"
---
apiVersion: v1
...
metadata:
  name: browser-uploads
  mountPath: /secure/uploads
```

#### Security Considerations
- **Directory Permissions**: Ensure upload directory has appropriate filesystem permissions
- **Path Validation**: All uploaded files are validated against the configured upload directory
- **Traversal Prevention**: Filenames with path separators or traversal sequences are rejected
- **Storage Management**: Implement cleanup policies for uploaded files to prevent disk exhaustion
- **Monitoring**: Monitor upload operations and validation failures for security incidents

#### Troubleshooting Common Issues
- **Upload not configured**: Ensure GATEWAY_BROWSER_UPLOAD_DIR is set to a valid directory path
- **Path traversal detected**: Check uploaded filenames for path separators or traversal sequences
- **Directory not found**: Verify upload directory exists and has proper permissions
- **File not found**: Ensure uploaded files exist in the configured upload directory
- **Permission denied**: Check filesystem permissions for the upload directory and files

**Section sources**
- [config.py](file://products/tool-gateway/src/tool_gateway/core/config.py)
- [browser_connector.py](file://products/tool-gateway/src/tool_gateway/tools/browser_connector.py)

### Expanded Browser Tool Surface Guide
**New Section** Comprehensive guide for understanding and managing the expanded browser tool surface with nine additional tools.

#### Tool Classification
The browser connector now supports fifteen tools total, categorized by risk level:

**Read Tier Tools** (no HITL approval required):
- `web.navigate`: Navigate to URLs within allowed origins
- `web.snapshot`: Capture DOM snapshot for interactive element addressing
- `web.screenshot`: Capture screenshots with credential masking
- `web.fill_credential`: Fill credentials from configured credential sets
- `web.extract`: Extract structured data from DOM elements
- `web.wait_for`: Wait for element state changes
- `web.hover`: Hover over elements for interaction
- `web.scroll`: Scroll within pages for content access
- `web.switch_frame`: Switch between frames (cross-origin denial enforced)

**Write Tier Tools** (require HITL approval):
- `web.click`: Click interactive elements
- `web.type`: Type text into form fields
- `web.select`: Select options from dropdown menus
- `web.press_key`: Press keyboard keys or combinations
- `web.upload_file`: Upload files from configured upload directory
- `web.evaluate`: Execute JavaScript expressions (mutation guard enforced)

#### Security Controls
- **Tier Classification**: Each tool is classified by risk level for appropriate approval workflows
- **Flow Binding**: All tools operate within approved skill-bound flows
- **Origin Validation**: Navigation and frame operations respect origin allowlists
- **Mutation Guards**: Write-tier tools inherit mutation protection mechanisms
- **Step Budget**: All tools consume step budget to prevent infinite loops
- **Deviation Guard**: Prevents interactions outside approved flows

#### Example Usage Patterns
```yaml
# Read-tier example: Extract data from dashboard
- tool: web.navigate
  parameters:
    url: "https://dashboard.internal"
    skill_id: "monitoring.dashboard"
...
# Write-tier example: Submit form with HITL approval
- tool: web.click
  parameters:
    ref: 1
    skill_id: "forms.submit"
  requires_approval: true
```

#### Monitoring and Diagnostics
- Monitor tool usage patterns by tier classification
- Track HITL approval rates for write-tier operations
- Validate flow binding and deviation guard enforcement
- Monitor step budget consumption and flow exhaustion
- Alert on unusual tool usage patterns or approval anomalies
