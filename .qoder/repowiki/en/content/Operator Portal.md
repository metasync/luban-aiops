...
## Update Summary
**Changes Made**
- Updated Operations Document Repository section to reflect SPEC-041 enhancements including deterministic summary lines in document lists, tabbed structured digest rendering with dedicated sections (Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, Raw JSON), bounded scrollable panes for digest and prose content, and improved foreign vs owner coverage tier handling
- Enhanced Documents view documentation with new tabbed interface structure and bounded pane functionality
- Updated troubleshooting guidance for SPEC-041 features including digest rendering issues and bounded pane problems
- Added comprehensive testing references for the new tabbed digest system and bounded scroll functionality
...
subgraph "Operations Document Repository (SPEC-041)"
...
CB[ModelCatalog API] --> CC[getModelCatalog Function]
CD[ComposerSelectionBar] --> CE[Extensible Control Strip]
CF[ModelSelect Component] --> CG[Dynamic Rendering Logic]
CH[Session Model State] --> CI[Pinned Model Persistence]
CJ[Fail-Open UX] --> CK[Graceful Degradation]
end
subgraph "Tier-Aware Confirmation System"
CL[APPROVAL_DECIDER_ROLES] --> CM[Role-Based Badge Display]
CN[Confirmation Tier Detection] --> CO[Operator vs Approver Badges]
CP[Role Verification] --> CQ[Approver Permission Checks]
CR[Tier-Aware UI] --> CS[Visual Status Indicators]
end
subgraph "Settings & Identity Panel"
CT[AuthContext] --> CU[Identity Information Display]
CV[Session Workspace] --> CW[Session Details]
CX[Platform Version] --> CY[Metadata Display]
CZ[Read-Only Interface] --> DA[Operational Insights]
end
subgraph "Evidence Persistence System"
DB[transcriptToTurns] --> DC[EvidenceFrame Mapping]
DD[EvidenceTurn Groups] --> DE[Request ID Attachment]
DF[Truncation Markers] --> DG[Payload Budget Handling]
DH[Live Stream Frames] --> DI[Unified Turn Model]
DJ[Replayed Evidence] --> DK[Consistent Rendering]
end
subgraph "Enhanced Navigation System"
DL[useNarrowViewport Hook] --> DM[Responsive Breakpoint Detection at 992px]
DN[Mobile Menu Button] --> DO[Dynamic ARIA Labels]
DP[Sidebar Collapsible 64px Rail] --> DQ[Drawer Integration]
DR[Content Spacing Management] --> DS[.view-container-inset Class]
end
subgraph "Build & Deployment"
DT[vite.config.ts] --> DU[package.json]
DV[Dockerfile] --> DW[Makefile]
DX[VERSION] --> DY[validate_version.py]
end
subgraph "Backend Services"
DZ[Agent Platform] --> EA[HITL Confirmations]
EB[Platform Gateway] --> EC[Identity Broker]
ED[Tool Gateway] --> EE[Policy Engine]
EF[Document Repository] --> EG[Shift Summaries]
end
end
```
...
- **Operations Document Repository**: Complete shift summary creation, management, viewing capabilities with SPEC-041 tabbed digest rendering and bounded scrollable panes, and client-side Markdown export functionality for shift summaries and operational documentation
...
### Operations Document Repository (SPEC-041)
...
Note over Documents : Operations Document Repository Initialization (SPEC-041)
...
- **Operations DocumentsView**: Complete shift summary management with session selection, document creation, publishing, viewing capabilities, tabbed structured digest rendering with dedicated sections, bounded scrollable panes, and client-side Markdown export functionality
...
The Documents view provides complete shift summary management with comprehensive session selection and document lifecycle management, enhanced with SPEC-041 readability improvements:
...
#### Operations Document Styling (SPEC-041)
...
- [global.css:365-407](file://products/operator-portal/web-ui/app/src/theme/global.css#L365-407)
...
#### Workspace Section  
- **Operations Documents**: Shift summary management with session selection, document creation, publishing, viewing capabilities, tabbed structured digest rendering, bounded scrollable panes, and client-side Markdown export functionality
...
- [useChatStream.ts:284-372](file://products/operator-portal/web-ui/app/src/stream/useChatStream.ts#L284-372)
- [transcript.ts:137-165](file://products/operator-portal/web-ui/app/src/chat/transcript.ts#L137-L165)
- [global.css:409-444](file://products/operator-portal/web-ui/app/src/theme/global.css#L409-L444)
- [roles.ts:35-35](file://products/operator-portal/web-ui/app/src/roles.ts#L35-L35)
...
The Documents view implements a comprehensive interface for managing immutable shift snapshots with session coverage and optional prose generation, enhanced with SPEC-041 tabbed digest rendering and bounded scrollable panes:
...
- **Operations Document Interface**: Accessible document management with proper ARIA labels, keyboard navigation, form controls for shift summary creation, tabbed structured digests with dedicated sections, bounded scrollable panes, and export button accessibility
...
- **Operations Document Interface**: Cross-browser support for document management with tabbed structured digests, bounded scrollable panes, and proper fallbacks when API services are unavailable
