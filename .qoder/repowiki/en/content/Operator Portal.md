...
- [toolNames.ts](file://products/operator-portal/web-ui/app/src/chat/toolNames.ts)
- [toolNames.test.ts](file://products/operator-portal/web-ui/app/src/chat/__tests__/toolNames.test.ts)
...
- [2026-08-31-chat-markdown-tool-identifier-rendering.md](file://docs/agentic-aiops-platform/release-notes/2026-08-31-chat-markdown-tool-identifier-rendering.md)
- [2026-08-31-dotted-canonical-tool-names-in-chat-prose.md](file://docs/agentic-aiops-platform/release-notes/2026-08-31-dotted-canonical-tool-names-in-chat-prose.md)
...
## Update Summary
**Changes Made**
- Updated Enhanced Markdown Rendering Interface section to reflect the enhanced tool name rendering system that now applies dotted canonical names consistently across all rendered surfaces including inline code spans and fenced blocks
- Revised Canonical Tool Name Display System documentation to document the simplified displayToolNames function implementation that removes complex sentinel-based shielding machinery
- Updated troubleshooting guidance for tool name rendering issues with new unified word-boundary substitution approach
- Enhanced testing coverage documentation for v0.27.5 changes that ensure consistent tool name formatting throughout chat responses and incident reports
...
I --> M[decoder.ts]
A --> N[AuditView.tsx]
A --> O[IncidentsView.tsx]
O --> P[Incident Detail Tabs]
P --> Q[Details Tab]
P --> R[Triage Tab]
P --> S[Dispatches Tab]
P --> T[Session Tab]
P --> U[Digest Data Tab]
A --> V[PermissionsView.tsx]
A --> W[SkillsView.tsx]
A --> X[ToolsView.tsx]
A --> Y[ApprovalsView.tsx]
A --> Z[DocumentsView.tsx]
A --> AA[SettingsView.tsx]
end
subgraph "Enhanced Incident Management System"
AB[IncidentsView Component] --> AC[Type Selector Radio Button]
AC --> AD[Severity Levels]
AC --> AE[Priority Categories]
AF[Searchable Incident List] --> AG[Real-time Updates]
AG --> AH[Advanced Filtering]
AI[Dedicated Tabs Interface] --> AJ[Incident Details]
AJ --> AK[Triage Analysis]
AK --> AL[Connector Dispatches]
AL --> AM[Session Integration]
AM --> AN[Digest Data Inspection]
AO[Enhanced Error Handling] --> AP[User Feedback]
AP --> AQ[Recovery Mechanisms]
end
subgraph "Enhanced Live Decision Sync System"
J --> AR[Bounded Polling 5s Interval]
AS[Change Detection] --> AT[Fingerprint Comparison]
AU[Time-Based Settle Window 300s] --> AV[Visibility/Focus Kick]
AW[Session Detail API] --> AX[Re-seed Timeline via reseedTurns]
end
subgraph "Authoritative Re-seed Mechanism"
AX[reseedTurns Method] --> AY[Same-Session Only Check]
AZ[Live Turns Replacement] --> BA[Cache Entry Replacement]
BB[No Stream Abort] --> BC[No Session Pointer Move]
BD[No-Op for Other Sessions] --> BE[Prevents Cache Poisoning]
end
subgraph "Turn-Anchored Confirmation System (SPEC-033)"
BF[attachConfirmations Function] --> BG[turn_index Validation]
BH[Anchored Target Selection] --> BI[Per-Exchange Anchoring]
BJ[Legacy Fallback] --> BK[Newest Turn Anchor]
BL[Synthetic Turn Creation] --> BM[Empty Transcript Handling]
BN[Pending Record Anchoring] --> BO[confirmationPending Flag]
end
subgraph "Enhanced Approvals Inbox System"
BP[useApprovalsInbox Hook] --> BQ[Real-time Polling 30s]
BR[getApprovalsInbox API] --> BS[Cross-session Discovery]
BT[ConfirmationRecord Types] --> BU[Decision Attribution]
BV[Race Resolution Handling] --> BW[409 Already Resolved]
BX[Client-Side Pagination] --> BY[10 Entries Per Page]
end
subgraph "Monotonic Session Workspace"
BZ[useSessionWorkspace Hook] --> CA[Monotonic Refresh Sequence]
CB[refreshSeqRef] --> CC[Prevents Race Conditions]
CD[Pinned Sessions] --> CE[Incident Deep Links]
CF[Active Session Persistence] --> CG[Session Storage]
CH[Inline Rename Support] --> CI[Session Title Updates]
CJ[Session ID Copy] --> CK[Clipboard Integration]
end
subgraph "Operations Document Repository (SPEC-041 + v0.23.2)"
CL[DocumentsView Component] --> CM[Session Selection]
CN[Create Shift Summary Dialog] CO[Document Creation]
CP[Document Publishing] --> CQ[Published State Management]
CR[Tabbed Digest Rendering] --> CS[Structured Sections]
CT[Foreign vs Owner Tier Handling] --> CU[Coverage Tier Display]
CV[Bounded Scrollable Panes] --> CW[Expand/Collapse Functionality]
CX[AI Narrative Panels] --> CY[Expanded by Default]
CZ[Improved Workflow Efficiency] --> DA[Operator Productivity]
end
subgraph "Enhanced Platform Components Health Dashboard"
DB[HealthDashboard Component] --> DC[Component Status Monitoring]
DD[Real-time Status Updates] --> DE[Gateway, Agent Service, LLM Runtime]
DF[Store Health Checks] --> DG[Session Store, State Store, Policy Bundle]
DH[Unified Status Indicators] --> DI[Ready, Degraded, Not Ready, Unavailable]
DJ[Tech Stack Visibility] --> DK[Framework & Server Versions]
DK[Backend Service Reporting] --> DL[Agent Platform, Gateway, Shared Contracts]
end
subgraph "Model Selection System"
DL[ModelCatalog API] --> DM[getModelCatalog Function]
DN[ComposerSelectionBar] --> DO[Extensible Control Strip]
DP[ModelSelect Component] --> DQ[Dynamic Rendering Logic]
DR[Session Model State] --> DS[Pinned Model Persistence]
DT[Fail-Open UX] --> DU[Graceful Degradation]
end
subgraph "Canonical Tool Name Display System (v0.27.5)"
DV[toolNames Module] --> DW[canonicalToolNames Function]
DX[displayToolNames Function] --> DY[Unified Word-Boundary Substitution]
DZ[Sanitized to Canonical Mapping] --> EA[Consistent Across All Surfaces]
EB[Longest-Match First Ordering] --> EC[Shared Prefix Resolution]
ED[Word Boundary Detection] --> EE[CommonMark Compliance]
EE[Inline Code Span Support] --> EF[Fenced Block Support]
EF[No Sentinel Machinery] --> EG[Simplified Implementation]
end
subgraph "Enhanced Navigation System"
EH[useNarrowViewport Hook] --> EI[Responsive Breakpoint Detection at 992px]
EJ[Mobile Menu Button] --> EK[Dynamic ARIA Labels]
EL[Sidebar Collapsible 64px Rail] --> EM[Drawer Integration]
EN[Content Spacing Management] --> EO[.view-container-inset Class]
end
subgraph "Build & Deployment"
EP[vite.config.ts] --> EQ[package.json]
ER[Dockerfile] --> EQ[Makefile]
ES[VERSION] --> ET[validate_version.py]
end
subgraph "Backend Services"
EU[Agent Platform] --> EV[HITL Confirmations]
EW[Platform Gateway] --> EX[Identity Broker]
EY[Tool Gateway] --> EZ[Policy Engine]
FA[Document Repository] --> FB[Shift Summaries]
FC[Health Monitor] --> FD[Component Status API]
FD[Tech Stack Reporting] --> FE[Version Tracking]
FF[Incident Service] --> FG[Automated Triage]
FG[Connector Integration] --> FH[External Systems]
end
end
```
...
- [ChatView.tsx:440-450](file://products/operator-portal/web-ui/app/src/chat/ChatView.tsx#L440-L450)
...
- [markdown.ts:89-112](file://products/operator-portal/web-ui/app/src/chat/markdown.ts#L89-L112)
- [toolNames.ts:19-46](file://products/operator-portal/web-ui/app/src/chat/toolNames.ts#L19-L46)
- [toolNames.ts:52-91](file://products/operator-portal/web-ui/app/src/chat/toolNames.ts#L52-L91)
- [markdown.test.ts:59-77](file://products/operator-portal/web-ui/app/src/chat/__tests__/markdown.test.ts#L59-L77)
- [toolNames.test.ts:1-47](file://products/operator-portal/web-ui/app/src/chat/__tests__/toolNames.test.ts#L1-L47)
...
- **Enhanced Markdown Renderer**: Comprehensive text formatting with XSS prevention, nested list support, sophisticated underscore handling for tool identifiers, and syntax highlighting support
- **Canonical Tool Name Display**: New presentation layer that rewrites sanitized tool names from underscore format to canonical dotted format in prose while preserving code regions, now enhanced with v0.27.5 to apply dotted canonical names consistently across all rendered surfaces including inline code spans and fenced blocks
...
- **Canonical Tool Name Display Interface**: Presentation layer that automatically rewrites tool names from sanitized underscore format to canonical dotted format in chat replies and incident reports while preserving code regions, now enhanced with v0.27.5 to consistently apply dotted canonical names across all rendered surfaces including inline code spans and fenced blocks
...
participant ToolNames as "Canonical Tool Names (v0.27.5)"
...
Note over React : Enhanced Canonical Tool Name Display (v0.27.5)
React->>ToolNames : Build canonicalToolNames map from catalog
ToolNames->>ToolNames : sanitizeToolName() converts dots to underscores
ToolNames->>ToolNames : canonicalToolNames() creates sanitized→canonical mapping
ToolNames->>ToolNames : displayToolNames() performs unified word-boundary substitution
ToolNames->>ToolNames : Rewrites tool names consistently across all surfaces (prose, inline code, fenced blocks)
ToolNames-->>React : Prose with canonical tool names (k8s.get_pods) everywhere
...
### Sophisticated Underscore Handling for Tool Identifiers

The markdown renderer now includes advanced underscore handling specifically designed for tool identifiers like `k8s_delete_pod`:

#### CommonMark-Compliant Emphasis Parsing
- **Word Boundary Detection**: Uses negative lookbehind and lookahead assertions to properly identify word boundaries around underscores
- **Emphasis Recognition**: Correctly parses emphasis markers (`*text*`, `_text_`) while protecting tool identifiers from unintended emphasis processing
- **Boundary Assertions**: Implements `(?<![a-zA-Z0-9])` and `(?![a-zA-Z0-9])` assertions to ensure underscores are only treated as emphasis markers when surrounded by appropriate boundaries
- **Tool Identifier Protection**: Preserves tool identifiers like `k8s_delete_pod` without applying emphasis formatting

### Canonical Tool Name Display Integration

The markdown rendering system now integrates with the enhanced canonical tool name display feature:

#### Presentation Layer Integration
- **Runtime Rewriting**: Tool names are rewritten from sanitized underscore format to canonical dotted format in prose
- **Consistent Surface Coverage**: Now applies dotted canonical names consistently across all rendered surfaces including inline code spans and fenced blocks
- **Longest-Match First**: Shared prefixes like `k8s_get_pods` vs `k8s_get_pod_logs` are resolved to the most specific registered name
- **Graceful Degradation**: Failed catalog fetches render text as written and retry on next mount

#### Simplified Implementation (v0.27.5)
- **Unified Word-Boundary Substitution**: The displayToolNames function now performs unified word-boundary substitution before markdown escaping
- **Removed Sentinel Machinery**: Complex sentinel-based shielding machinery has been removed in favor of simpler, more reliable approach
- **Consistent Formatting**: Ensures consistent tool name formatting (e.g., k8s_get_pods → k8s.get_pods) throughout chat responses and incident reports
- **Enhanced Reliability**: More robust handling of edge cases and improved performance

#### Tool Name Mapping
- **Sanitized to Canonical Mapping**: Built once per page session from existing `/api/v1/tools` catalog
- **Collision Handling**: Ambiguous sanitized collisions are skipped rather than guessed
- **Control Character Stripping**: Null characters are stripped from input to prevent sentinel collision
- **Mid-Stream Protection**: Unclosed fences are left untouched rather than half-rewritten
...
- **Underscore Handling Tests**: Tests verify proper handling of tool identifiers and emphasis markers with boundary detection
- **Tool Name Display Tests**: Tests verify canonical tool name rewriting and code region protection, now including v0.27.5 enhancements for consistent surface coverage
...
**Updated** The markdown rendering system now includes sophisticated underscore handling for tool identifiers like k8s_delete_pod, implementing CommonMark-compliant emphasis parsing with proper word boundary detection using negative lookbehind/lookahead assertions. Code content protection via sentinel characters ensures fenced code blocks and inline code spans remain isolated from subsequent markdown processing passes. **Critical Enhancement**: The underscore handling prevents unintended emphasis processing of tool identifiers while maintaining proper emphasis parsing for regular text content. **Enhanced Security**: The sentinel character system provides robust protection for code content during markdown processing, preventing any markdown transformations from affecting code blocks or inline code spans. **v0.18.1 Enhancement**: The nested list handling addresses the legacy column-0-only passes that dropped indented sub-bullets, providing proper nesting-aware block processing that maintains list integrity and semantic structure. **Critical Enhancement**: Pod log quoting improvements now render log excerpts in fenced code blocks with proper line breaks, replacing the previous JSON-serialized string format that was difficult to read. **v0.27.4 Enhancement**: The system now includes a new presentation layer for canonical tool name display in chat interface and incident reports, providing runtime rewriting of tool names from sanitized underscore format to canonical dotted format in prose while preserving code regions. **v0.27.5 Enhancement**: The tool name rendering system has been enhanced to apply dotted canonical names consistently across all rendered surfaces including inline code spans and fenced blocks, with the displayToolNames function rewritten to remove complex sentinel-based shielding machinery and perform unified word-boundary substitution before markdown escaping, ensuring consistent tool name formatting throughout chat responses and incident reports.
...
- [toolNames.test.ts:1-94](file://products/operator-portal/web-ui/app/src/chat/__tests__/toolNames.test.ts#L1-L94)
...
- [2026-08-31-chat-markdown-tool-identifier-rendering.md:1-59](file://docs/agentic-aiops-platform/release-notes/2026-08-31-chat-markdown-tool-identifier-rendering.md#L1-L59)
- [2026-08-31-dotted-canonical-tool-names-in-chat-prose.md:17-60](file://docs/agentic-aiops-platform/release-notes/2026-08-31-dotted-canonical-tool-names-in-chat-prose.md#L17-L60)
...
- **Enhanced Markdown Styling**: Customizable appearance for nested lists, ordered list numbering, fenced code blocks with pod log display, sophisticated underscore handling for tool identifiers, and canonical tool name display with runtime rewriting
- **Canonical Tool Name Display Styling**: Customizable appearance for tool name rewriting with proper code region protection and longest-match first ordering, now enhanced with v0.27.5 to consistently apply dotted canonical names across all rendered surfaces including inline code spans and fenced blocks
...
- **Enhanced Markdown**: Accessible nested list rendering with proper semantic structure, keyboard navigation support, sophisticated underscore handling for tool identifiers, and canonical tool name display with runtime rewriting
- **Canonical Tool Name Display**: Accessible tool name rewriting with proper ARIA labels for sanitized to canonical conversions and code region protection, now enhanced with v0.27.5 to consistently apply dotted canonical names across all rendered surfaces including inline code spans and fenced blocks
...
- **Enhanced Markdown**: Cross-browser support for nested list rendering, fenced code blocks with pod log display, sophisticated underscore handling for tool identifiers, and canonical tool name display with proper fallbacks for older browsers
- **Canonical Tool Name Display**: Cross-browser support for runtime tool name rewriting with graceful degradation when catalog service is unavailable, now enhanced with v0.27.5 to consistently apply dotted canonical names across all rendered surfaces including inline code spans and fenced blocks
...
- **Updated**: Verify that sophisticated underscore handling is working correctly for tool identifiers like k8s_delete_pod with CommonMark-compliant emphasis parsing
- **Updated**: Check that code content protection via sentinel characters is properly isolating fenced code blocks and inline code spans from subsequent markdown processing passes
- **Updated**: Verify that negative lookbehind/lookahead assertions are properly detecting word boundaries around underscores
- **Updated**: Test underscore handling scenarios to ensure tool identifiers are protected from unintended emphasis processing
- **v0.27.4 Enhancement**: Verify that canonical tool name display is working correctly with runtime rewriting from sanitized underscore format to canonical dotted format in prose while preserving code regions
- **v0.27.5 Enhancement**: Verify that the enhanced tool name rendering system now applies dotted canonical names consistently across all rendered surfaces including inline code spans and fenced blocks, with the displayToolNames function rewritten to remove complex sentinel-based shielding machinery and perform unified word-boundary substitution before markdown escaping
...
- **Enhanced Markdown Rendering (v0.18.1)**: Nested list support with proper ordered/unordered list handling, pod log quoting in fenced code blocks, sophisticated underscore handling for tool identifiers like k8s_delete_pod with CommonMark-compliant emphasis parsing, and code content protection via sentinel characters ensuring fenced code blocks and inline code spans remain isolated from subsequent markdown processing passes
