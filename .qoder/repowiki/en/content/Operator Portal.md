...
## Update Summary
**Changes Made**
- Added new presentation layer for canonical tool name display in chat interface and incident reports with runtime rewriting from sanitized underscore format to canonical dotted format
- Enhanced markdown rendering system with sophisticated underscore handling for tool identifiers like k8s_delete_pod using CommonMark-compliant emphasis parsing
- Implemented code content protection via sentinel characters to ensure fenced code blocks and inline code spans remain isolated from subsequent markdown processing passes
- Updated Enhanced Markdown Rendering Interface section with new underscore handling capabilities and tool name display features
- Enhanced troubleshooting guidance for markdown rendering issues including underscore handling, code block protection, and canonical tool name display
- Added comprehensive documentation for the new toolNames.ts module and its integration with ChatView component
...
subgraph "Canonical Tool Name Display System"
DV[toolNames Module] --> DW[canonicalToolNames Function]
DX[displayToolNames Function] --> DY[Runtime Rewriting]
DZ[Sanitized to Canonical Mapping] --> EA[Code Region Protection]
...
EF[useNarrowViewport Hook] --> EG[Responsive Breakpoint Detection at 992px]
EH[Mobile Menu Button] --> EI[Dynamic ARIA Labels]
EJ[Sidebar Collapsible 64px Rail] --> EK[Drawer Integration]
EL[Content Spacing Management] --> EM[.view-container-inset Class]
end
subgraph "Build & Deployment"
EN[vite.config.ts] --> EO[package.json]
EP[Dockerfile] --> EQ[Makefile]
ER[VERSION] --> ES[validate_version.py]
end
subgraph "Backend Services"
ET[Agent Platform] --> EU[HITL Confirmations]
EV[Platform Gateway] --> EW[Identity Broker]
EX[Tool Gateway] --> EY[Policy Engine]
EZ[Document Repository] --> FA[Shift Summaries]
FB[Health Monitor] --> FC[Component Status API]
FC[Tech Stack Reporting] --> FD[Version Tracking]
FE[Incident Service] --> FF[Automated Triage]
FF[Connector Integration] --> FG[External Systems]
end
end
```
...
- **Canonical Tool Name Display**: New presentation layer that rewrites sanitized tool names from underscore format to canonical dotted format in prose while preserving code regions
...
- **Canonical Tool Name Display Interface**: Presentation layer that automatically rewrites tool names from sanitized underscore format to canonical dotted format in chat replies and incident reports while preserving code regions
...
participant ToolNames as "Canonical Tool Names"
...
Note over React : Canonical Tool Name Display
...
ToolNames->>ToolNames : displayToolNames() rewrites prose with longest-match first
ToolNames->>ToolNames : Code regions shielded from rewriting (fenced blocks, inline spans)
ToolNames-->>React : Prose with canonical tool names (k8s.list_pods)
...
#### Code Content Protection via Sentinel Characters
- **Sentinel Character System**: Uses special sentinel characters to mark code blocks and inline code spans during processing
- **Isolation Mechanism**: Ensures fenced code blocks and inline code spans remain completely isolated from subsequent markdown processing passes
- **Protection Strategy**: Prevents emphasis parsing, underscore processing, and other markdown transformations within code contexts
- **Restoration Process**: Safely removes sentinel characters after processing is complete, restoring original code content

#### Enhanced Processing Pipeline
- **Initial Code Detection**: Scans content to identify and isolate code blocks and inline code spans using sentinel markers
- **Protected Processing**: Applies markdown transformations only to non-code content, preserving code integrity
- **Underscore Handling**: Processes underscores and emphasis markers with proper boundary detection outside of protected code regions
- **Final Restoration**: Removes sentinel characters and restores original code content while maintaining all markdown transformations

### Canonical Tool Name Display Integration

The markdown rendering system now integrates with the new canonical tool name display feature:

#### Presentation Layer Integration
- **Runtime Rewriting**: Tool names are rewritten from sanitized underscore format to canonical dotted format in prose
- **Code Region Protection**: Fenced code blocks and inline code spans preserve the original sanitized format for configuration surfaces
...
- **Tool Name Display Tests**: Tests verify canonical tool name rewriting and code region protection
...
- **Canonical Tool Name Display Styling**: Customizable appearance for tool name rewriting with proper code region protection and longest-match first ordering
...
- **Canonical Tool Name Display**: Accessible tool name rewriting with proper ARIA labels for sanitized to canonical conversions and code region protection
...
- **Canonical Tool Name Display**: Cross-browser support for runtime tool name rewriting with graceful degradation when catalog service is unavailable
