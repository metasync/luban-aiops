...
## Update Summary
**Changes Made**
- Added nginx routing configuration for health check endpoints (/health/) to properly route probes to platform gateway service on port 8000, fixing Settings Platform pane health monitoring in v0.23.3
- Updated Deployment Guide section to document the new health endpoint routing configuration
- Enhanced Troubleshooting Guide with specific guidance for health endpoint connectivity issues
- Updated Platform Components Health Dashboard section to reflect proper backend integration through nginx routing
...
23. [Incident Triage and Deep Linking](#incident-triage-and-deep-linking)
...
subgraph "Platform Components Health Dashboard"
...
CH[Status Indicators] --> CI[Online, Degraded, Offline States]
CI[Alert Notifications] --> CJ[Automated Alerting]
end
...
DK[Health Dashboard] --> DL[Component Status Monitoring]
end
...
- **Settings & Identity Panel**: Read-only panel displaying identity information, session details, platform metadata, and comprehensive Platform Components Health Dashboard with real-time status monitoring
- **Platform Components Health Dashboard**: Live monitoring table showing real-time status of critical platform components including gateway, agent service, LLM runtime, session store, state store, and policy bundle
...
- **Platform Components Health API Client**: Real-time component status monitoring via health check endpoints routed through nginx to platform gateway service on port 8000
...
- **Settings & Debug Panel**: Configuration management and debugging tools with read-only operational insights and health dashboard
...
- **Operations Document Interface**: Complete shift summary management with session selection, document creation, publishing, viewing capabilities, tabbed structured digest rendering with dedicated sections (Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, Raw JSON), bounded scrollable panes, v0.23.2 enhanced narrative panels that open expanded by default, and client-side Markdown export functionality
- **Platform Components Health Dashboard Interface**: Real-time monitoring table with component status indicators, alert notifications, and detailed health information
...
- **Tabbed Structured Digest Rendering**: Dedicated sections for Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, and Raw JSON
...
participant HealthDash as "Health Dashboard"
...
Documents->>Documents : Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, Raw JSON tabs
...
Note over HealthDash : Platform Components Health Dashboard Initialization
React->>HealthDash : HealthDashboard component initialization
HealthDash->>Nginx : GET /health/live (v0.23.3 routing fix)
Nginx->>Gateway : Proxy to platform-gateway : 8000
Gateway->>HealthAPI : Fetch component health status
HealthAPI-->>HealthDash : Component status (gateway, agent service, LLM runtime, stores)
HealthDash->>HealthDash : Real-time status updates with polling
...
- **SettingsView**: Read-only Session & Identity panel displaying operational insights and comprehensive Platform Components Health Dashboard with real-time component status monitoring
- **Platform Components Health Dashboard**: Live monitoring table showing real-time status of gateway, agent service, LLM runtime, session store, state store, and policy bundle with alert notifications
...
- **Raw JSON Tab**: Stored digest verbatim for artifact inspection with graceful degradation for pre-SPEC-040 documents
...
The Settings view now includes a comprehensive Platform Components Health Dashboard that provides real-time monitoring of critical platform components:

#### Real-time Component Monitoring
- **Live Status Updates**: Continuous polling of component health status with configurable refresh intervals
- **Component Coverage**: Monitors gateway, agent service, LLM runtime, session store, state store, and policy bundle
- **Status Indicators**: Visual indicators showing Online, Degraded, and Offline states for each component
- **Performance Metrics**: Response time monitoring and availability tracking for capacity planning
- **v0.23.3 Enhancement**: Health check requests are now properly routed through nginx to the platform gateway service on port 8000, ensuring reliable health monitoring functionality

#### Alert Notification System
- **Automated Alerts**: Real-time notifications when components become unhealthy or degraded
- **Escalation Policies**: Configurable alert thresholds and notification channels
- **Historical Alert Log**: Complete audit trail of health events and their resolutions
- **Integration Capabilities**: Integration with existing alerting systems and notification platforms

#### Health Dashboard Interface
- **Interactive Table Layout**: Clean, responsive table displaying component names, current status, response times, and last checked timestamps
...
- **Platform Components Health Dashboard**: Styled monitoring table with status indicators, alert notifications, and responsive design
...
- **Digest Panels**: Tabbed interface with structured display for Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, and Raw JSON sections
...
#### Platform Components Health Dashboard Styling
- **Monitoring Table**: Clean table layout with component names, status indicators, response times, and last checked timestamps
- **Status Indicators**: Color-coded badges for Online (green), Degraded (yellow), and Offline (red) states
...
- **Settings**: Read-only Session & Identity panel with operational insights and Platform Components Health Dashboard
...
- **Settings**: Available to all users for operational insights and health monitoring
...
**Updated** The Settings view restoration as a read-only Session & Identity panel (R-6) continues to provide administrators with essential operational insights without introducing new security risks. The view continues to leverage existing AuthContext and session workspace state to display identity information, session details, and platform metadata in a consolidated dashboard format, now enhanced with comprehensive Platform Components Health Dashboard for real-time monitoring of critical platform components. **Critical Enhancement**: The read-only nature of the panel continues to ensure that operational awareness does not introduce unintended modification capabilities, maintaining the principle of least privilege while providing comprehensive situational awareness. **New Feature**: The Platform Components Health Dashboard provides real-time monitoring of gateway, agent service, LLM runtime, session store, state store, and policy bundle with live status indicators, alert notifications, and comprehensive health reporting for proactive platform management. **v0.23.3 Enhancement**: Health check requests are now properly routed through nginx to the platform gateway service on port 8000, fixing Settings Platform pane health monitoring functionality.
...
The Operator Portal now includes a comprehensive Platform Components Health Dashboard integrated into the Settings view, providing real-time monitoring and alerting for critical platform components.

### Health Dashboard Architecture

The Platform Components Health Dashboard provides a centralized interface for monitoring the health and status of essential platform components:

#### Component Coverage
- **Gateway Service**: Platform gateway monitoring with response time and availability tracking
- **Agent Service**: Agent platform service health with execution status and performance metrics
- **LLM Runtime**: Large Language Model runtime monitoring with model availability and response quality
- **Session Store**: Session persistence layer health with database connectivity and performance
- **State Store**: State management service monitoring with data consistency and availability
...
The dashboard provides clear visual indicators for component health status:

#### Status Categories
- **Online**: Component is fully operational with normal response times and zero errors
- **Degraded**: Component is experiencing performance issues or elevated error rates
- **Offline**: Component is completely unavailable or experiencing critical failures
- **Unknown**: Unable to determine component status due to network or monitoring issues

#### Visual Presentation
- **Color-coded Badges**: Green for Online, yellow for Degraded, red for Offline, gray for Unknown
...
- **Status Columns**: Real-time status, response time, and last checked information
...
The Platform Components Health Dashboard provides significant operational advantages:
...
**New Feature**: The Platform Components Health Dashboard provides comprehensive real-time monitoring of critical platform components including gateway, agent service, LLM runtime, session store, state store, and policy bundle with live status indicators, alert notifications, and detailed health reporting for proactive platform management. **v0.23.3 Enhancement**: Health check requests are now properly routed through nginx to the platform gateway service on port 8000, fixing Settings Platform pane health monitoring functionality.
...
**Updated** The multi-session workspace now includes enhanced monotonic refresh sequences to prevent race conditions during concurrent decision processing, ensuring data consistency across multiple approval operations. The session workspace continues to include comprehensive stale session handling with missing reference tracking to prevent errors from deleted or expired sessions. These improvements ensure more reliable session management and better data integrity. **Enhanced Feature**: The Settings view continues to provide operational context that complements session management by displaying current session information and identity details in a consolidated dashboard format, now enhanced with comprehensive Platform Components Health Dashboard for real-time monitoring. **Enhanced Feature**: The enhanced approvals inbox continues to integrate with session management to provide cross-session confirmation management capabilities for designated approvers. **Enhanced Feature**: The Operations Document Repository continues to integrate with session management to provide cross-session analysis capabilities through shift summary creation and foreign session metadata inclusion. **Critical Enhancement**: The live decision sync system continues to work seamlessly with multi-session management to ensure external decisions are properly synchronized to the correct active session context.
...
- **v0.23.3 Enhancement**: Health check endpoint routing for `/health/` paths to platform gateway service on port 8000, fixing Settings Platform pane health monitoring functionality
...
- **Operations Document Styling**: Customizable appearance for document cards, tabbed structured digests with dedicated sections (Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, Raw JSON), bounded scrollable panes, prose panels, create dialogs, export button, and v0.23.2 enhanced narrative panels that open expanded by default
- **Platform Components Health Dashboard Styling**: Customizable appearance for monitoring table with status indicators, alert notifications, and responsive design
...
- **Settings View Styling**: Customizable appearance for the read-only Session & Identity panel with operational insights dashboard and Platform Components Health Dashboard
...
- **Platform Components Health Dashboard**: Accessible health monitoring interface with proper ARIA labels, keyboard navigation, status announcements, and alert notifications
...
- **Settings View**: Accessible read-only Session & Identity panel with proper ARIA labels and keyboard navigation for operational insights and health monitoring
...
- **Platform Components Health Dashboard**: Cross-browser support for health monitoring interface with proper fallbacks when health API services are unavailable
...
- Check that digest panels display correctly with tabbed structured sections (Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, Raw JSON)
...
**Solution**:
- Verify that DigestPanel component is properly rendering all tabs (Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, Raw JSON)
...
- Check that raw JSON tab shows stored digest verbatim for artifact inspection
...
### Platform Components Health Dashboard Issues
...
- Verify that component status indicators are displaying correctly
- Check that alert notification system is properly configured and functioning
- Test health dashboard with different component states (online, degraded, offline)
...
### Platform Components Health Dashboard API Issues
...
- Check that health responses include required fields (component name, status, response time, last checked)
...
### Platform Components Health Dashboard Styling Issues

**Problem**: Health dashboard layout or styling problems
**Solution**:
- Verify that monitoring table has proper flexbox styling with component names, status indicators, and response times
- Check that status badges display correctly with proper color coding (green for online, yellow for degraded, red for offline)
- Ensure that alert notifications appear in correct positions with proper styling
...
- **Settings View Restoration**: Read-only Session & Identity panel providing operational insights using existing AuthContext and session workspace state, now enhanced with comprehensive Platform Components Health Dashboard
- **Platform Components Health Dashboard**: Real-time monitoring of critical platform components including gateway, agent service, LLM runtime, session store, state store, and policy bundle with live status indicators, alert notifications, and comprehensive health reporting
...
- **SPEC-041 Enhancements**: Tabbed structured digest rendering with dedicated sections (Handover, Sessions, Confirmations, Executions, Evidence & transcript, Open items, Raw JSON), bounded scrollable panes for digest and prose content, and improved foreign vs owner coverage tier handling for enhanced operator readability and workflow efficiency
