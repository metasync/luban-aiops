...
## Update Summary
**Changes Made**
- Enhanced kernel middleware with evidence parameter redaction for tool_call frames and session service with title secret masking
- Added specialized secret_params.py module providing three distinct masking postures: change-request projection, evidence frame preservation, and authoring trace parameterization
- Updated session service to mask credentials in server-minted titles using multi-layer heuristic approach
- Integrated evidence parameter redaction into kernel middleware to prevent credential leakage in evidence panels
- Added comprehensive test coverage for secret parameterization including query URL redaction and nested structure processing
