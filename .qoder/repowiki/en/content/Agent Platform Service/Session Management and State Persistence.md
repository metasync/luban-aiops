...
## Update Summary
**Changes Made**
- Added Postgres session store backend alongside existing Redis and memory backends (SPEC-016)
- Updated architecture diagrams to reflect multi-backend support with fail-open fallback behavior
- Enhanced data models section to include Postgres schema and idle-TTL refresh mechanism
- Added bounded sweep mechanism documentation for expired session cleanup
- Updated configuration section to document SESSION_STORE_BACKEND and SESSION_DB_URL environment variables
- Expanded disaster recovery procedures to cover Postgres-specific scenarios
- Added comprehensive test coverage documentation for all three backends
...
- Services layer implements session orchestration and storage abstraction with pluggable backends
- Schemas define API contracts and request/response shapes
- Tests validate behavior for all session services and storage backends
...
participant Factory as "build_session_store()"
...
Service->>Factory : "get session store"
Factory->>Factory : "select backend by env"
Factory->>Store : "initialize backend"
Store-->>Factory : "store instance or fallback"
Service->>Store : "persist session"
Store-->>Service : "success or error"
Service-->>Routes : "session response"
Routes-->>Client : "HTTP 201 + session"
Note over Factory,Store : "Fail-open : Postgres/Redis failure → InMemory"
```

**Diagram sources**
- [session_store.py:536-615](file://products/agent-platform/src/agent_service/services/session_store.py#L536-L615)
- [session_store.py:523-533](file://products/agent-platform/src/agent_service/services/session_store.py#L523-L533)
...
BackendType --> |Redis| RedisPersist["Redis Storage"]
...
### Postgres Backend Implementation Details
The Postgres backend implements SPEC-016 requirements with sophisticated TTL management:

- **Idle-TTL Refresh**: Every read operation automatically updates `last_accessed_at` timestamp using a single UPDATE statement that also checks TTL validity
- **Bounded Sweep Mechanism**: Expired rows are opportunistically cleaned up during write operations with a limit of 100 rows per sweep to prevent long-running queries
...
- [session_store.py:389-420](file://products/agent-platform/src/agent_service/services/session_store.py#L389-L420)
- [session_store.py:422-442](file://products/agent-platform/src/agent_service/services/session_store.py#L422-L442)
...
- Abstraction: SessionStore protocol defines consistent interface allowing pluggable backends
