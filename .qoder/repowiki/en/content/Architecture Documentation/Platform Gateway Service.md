...
## Introduction
The Platform Gateway Service is the portal-facing edge service for the Luban AIOps platform. It authenticates portal users via JWT verification, enforces deny-by-default action policies, proxies chat and session requests to the agent-platform service, and mediates short-lived delegated tokens through the identity-broker for downstream tool access. It exposes health, metrics, and runtime endpoints and maintains request correlation across hops.

Key responsibilities:
- Verify portal bearer tokens (issuer/audience JWKS validation; audience bound to platform-gateway).
- Enforce deny-by-default policy bundle on every portal-facing action (e.g., chat, sessions:*).
...
- [policy-default.yaml:1-63](file://products/platform-gateway/src/platform_gateway/policies/policy-default.yaml#L1-L63)
- [config.py:1-91](file://products/platform-gateway/src/platform_gateway/core/config.py#L1-L91)
...
The gateway sits between the portal and backend services. It authenticates users, authorizes actions, proxies requests, and obtains delegated tokens for tool execution paths.
...
Portal->>GW : POST /api/v1/chat or GET /api/v1/chat/stream
...
- The router aggregates health, runtime, auth, identity, sessions, and chat routes.
- Chat and Sessions routes enforce identity and policy before delegating to gateway service functions.
...
- Default bundle grants chat and session operations to operators/developers and read-only observers limited to read actions.
...
- Validate environment variables for JWKS URL, audiences, and policy path.
...
## Conclusion
The Platform Gateway Service cleanly separates portal-facing security and control-plane concerns from tool execution capabilities. It enforces strong authentication and authorization, proxies to agent-platform securely with least-privilege delegated tokens, and provides robust observability and operational signals. The design aligns with workspace model boundaries and enables future growth without compromising trust or clarity.
