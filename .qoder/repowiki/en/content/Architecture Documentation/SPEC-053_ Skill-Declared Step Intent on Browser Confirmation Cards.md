...
**Section sources**
- [spec.md:19-51](file://docs/specs/SPEC-053-skill-declared-step-intent/spec.md#L19-L51)

## Project Structure
This spec touches four product layers plus shared contracts and a sample:
- Skills contract and ingestion (skills-hub): declares and persists `flow_intent`.
...
- Portal UI (operator-portal): renders the intent as a prominent decision line on the confirmation card.
- Shared schemas: add `flow_intent` to the skill envelope and flow-summary schema; bump stream contract version.
- Sample: demonstrates the key in a real write-class skill.
...
## Core Components
- Skill envelope model: Adds `flow_intent: str | None` with length constraints and explicit allowance in the Pydantic model.
- Skill contract schema: Adds optional `flow_intent` string property to the canonical skill schema.
- Gateway FlowState: Carries `flow_intent` alongside title/description and includes it in serialization for the flow payload.
...
- Portal models and rendering: Maps wire snake_case to camelCase and renders the intent as escaped text in the confirmation card headline block.
- Sample skill: Declares `flow_intent` describing the single write-tier action.
...
1. Skills hub ingests and validates `flow_intent` from skill frontmatter and persists it verbatim.
...
4. Operator portal decodes the stream and durable records, mapping `flow_intent` to `flowIntent`, and renders it as a plain-text decision line above technical details.
...
OP->>OP : ConfirmationCardView renders intent line
```
...
- Contract: `skill.schema.json` adds optional `flow_intent` string with length bounds and description.
- Ingestion/persistence: Per plan, ingestion accepts `flow_intent` in allowed keys and validates presence/length; both store backends persist and round-trip the field.
...
- `bind_flow` populates `flow_intent` from the fetched skill’s frontmatter.
...
- `FlowContext` stores `flow_intent` and reads it from the incoming flow dict during `record()`.
...
- `ConfirmationCardView` renders `flowSummary.flowIntent` as escaped text within the `.confirm-flow` headline block, above technical details.
...
V->>V : Render intent line as text
```
...
- `ResetUserPassword.md` declares `flow_intent` describing the single write-tier “Confirm reset” click and bumps its version.
- The sample docs note that the card now leads with the authored intent.
...
- Validation errors on skill ingestion:
  - Check length limits and that `web_target` is present when `flow_intent` is declared; ensure schema envelope remains closed (`additionalProperties: false`).
- Security concerns:
...
## Conclusion
SPEC-053 delivers a small, robust enhancement to the browser confirmation card by surfacing a skill-authored intent line where operators actually make decisions. It extends the existing flow-summary pipeline with one optional field, preserves backward compatibility, and avoids any security or policy surface expansion. The password-reset sample demonstrates the feature end-to-end, and the layered implementation across skills-hub, tool-gateway, agent-platform, and operator-portal ensures consistent rendering for live and durable views.
