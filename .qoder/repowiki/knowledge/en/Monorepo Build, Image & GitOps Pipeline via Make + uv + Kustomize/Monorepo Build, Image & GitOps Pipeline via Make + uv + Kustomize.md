...
- Version lock: `VERSION` file at repo root (`0.36.1`) is the single source of truth; `make validate-version` enforces that every product's `pyproject.toml` version matches it via `shared/shared-contracts/scripts/validate_version.py`.
