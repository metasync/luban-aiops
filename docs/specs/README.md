...
| `SPEC-041` | Documents readability and digest reference — operator-facing digest/vocabulary reference guide, tabbed structured digest rendering in the drawer (handover default, raw JSON preserved), bounded scrollable digest and prose panes, and a deterministic counts-only document summary shown in the Mine/Published lists (third R5 slice; extends SPEC-039/040) | `draft` |
