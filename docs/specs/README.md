...
| `SPEC-039` | Shift-summary artifacts — durable handover artifacts with a deterministic digest assembled from the durable records (sessions, confirmations, executions, evidence), two-tier own/metadata-only foreign coverage behind `shifts:summarize`, optional clearly-labeled digest-only prose layer, provenance anchoring, and a portal Shift summaries view (session-handover spike Option B; first R5 slice; extends SPEC-022/025/031/037) | `draft` |
