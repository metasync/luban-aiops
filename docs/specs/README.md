...
| `SPEC-055` | Develop-as-you-go skill graduation — the graduation destination of the A→B→C program (implements ADR-0009): a durable authoring-trace store (dual-backend, separate from the 30-day execution sweep, secret-safe parameterized steps referencing `execution_id`/`confirm_id`) captured at the approval seam, an executable-flow skill class declaring `risk_class: write` decoupled from `web_target` with a machine-readable replay step list, deterministic human-merge graduation with blast-radius re-validation (one new `session:skill_graduate` action + `skill_graduated` audit event, never auto-publish), and one-gate secret-safe replay under SPEC-051's authority with credentials resolved from credential-set references; phases ADR-0007 rather than reversing it (seventeenth R5 slice; depends on SPEC-054; extends SPEC-014/037/038/044/045/049/050/051) | `approved` |
