...
| `SPEC-042` | Portal dependency hygiene — migration off every deprecated antd v6 API (Drawer `width` → `size`, Alert `message` → `title`), a zero-tolerance vitest deprecation guard, a managed component refresh per the 2026-08-28 upgrade check (vite 8 + plugin-react 6, vitest 4, TypeScript 5.9, jsdom 30), and the React 19 migration (fourth R5 slice; extends SPEC-023) | `draft` |
