...
| `SPEC-047` | Audit summary drill-down and readability — every SPEC-046 aggregate value clickable into the Events tab under merged filters (enabled by one additive `outcome` filter dimension on the existing audit routes), percentage + bar proportion on every bucket row, antd statistic row for the headline numbers, collapsible bucket sections; multi-tab-per-report rejected (ninth R5 slice; extends SPEC-013/046) | `delivered` |
