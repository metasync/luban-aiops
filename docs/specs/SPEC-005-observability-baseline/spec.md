...
- 2026-08-14: reconciliation of the `prometheus-fastapi-instrumentator` deviation promised in the previous entry — recorded here per the delivered-spec rule (corrections go in the changelog rather than rewriting frozen requirements). The body wording in the Decision section, R-1 criteria ("registered via `prometheus-fastapi-instrumentator`", "each `pyproject.toml` gains `prometheus-fastapi-instrumentator`"), and the Impact dependency line is superseded as follows: the always-on `/metrics` surface is implemented directly with `prometheus_client` (minimal RED middleware: `http_requests_total{method,handler,status}` counter + `http_request_duration_seconds{method,handler}` histogram, templated `handler` label from the matched route, `/metrics` itself excluded); each `pyproject.toml` depends on `prometheus-client`, not `prometheus-fastapi-instrumentator`. Behavior and acceptance outcomes are unchanged. The living-state reference `shared/shared-contracts/observability-conventions.md` was updated to match on the same date; `plan.md` retains the original approved plan as a historical record.
