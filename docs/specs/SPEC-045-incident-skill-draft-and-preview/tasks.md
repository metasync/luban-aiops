...
- [ ] `skill_draft.py` extended (not forked):
      ...
      the builder
- [ ] Skeleton builder extension: facts-only skeleton from the
...
- [ ] audit-service: `incident_skill_draft_generated` in the event
      enum (`schemas/audit.py`) + shared `audit-event.schema.json`
      with the SPEC-029 parity-guard members updated
- [ ] agent-platform: emit `incident_skill_draft_generated` on the
...
- [ ] `api/incidents.ts`: `createIncidentSkillDraft()` on the shared
...
- [ ] `IncidentsView.tsx` detail toolbar: **Draft as skill** beside
      ...
      the preview
- [ ] Vitest: visibility per role, busy state, the five error
      toasts, preview-open on success

## R-5: Shared skill-draft preview (W-4)

- [ ] `SkillDraftPreview` modal: bounded scrollable rendered
...
- [ ] **Download .md** primary (SPEC-040 R-4 Blob download, raw
...
- [ ] Browser live check: incident-entry draft + preview + download
      ...
      through the preview
- [ ] Living-state docs: `skills-guide.md`, `portal-user-guide.md`,
      `incident-guide.md`, `authorization-matrix.md`
      (`configuration-reference.md` gains no new knobs)
- [ ] CHANGELOG 0.27.0 + release note + index; commit/tag/push per
      the house train
