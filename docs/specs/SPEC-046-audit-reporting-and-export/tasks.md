...
- [ ] `AuditStore.summarize(filters)` on the protocol;
...
- [ ] `AuditSummary` response shape: `total_events`, `window` echo
      ...
      absent)
- [ ] `GET /api/v1/audit/summary` route: query-route structure
...
- [ ] Tests: in-memory ↔ fake-driver Postgres summarize parity,
...
- [ ] `AUDIT_EXPORT_MAX_ROWS` knob (default 10 000, validated
      positive) in `core/config.py`
- [ ] `GET /api/v1/audit/export`: `StreamingResponse` (`text/csv`)
...
- [ ] `audit_export_generated` log line (caller, rows) +
      `audit_exports_total` counter
- [ ] Tests: cap truncation (cap + 1 rows → exactly cap rows,
...
- [ ] `GET /api/v1/audit/summary` proxy: existing `audit:read`
...
- [ ] `GET /api/v1/audit/export` proxy: byte pass-through with
...
- [ ] Tests: policy denial for ungranted roles on both routes,
...
- [ ] audit-service `test_contracts.py`: summary-model ↔ schema
      binding beside the enum parity class
- [ ] Portal `views/audit/constants.ts`: `EVENT_TYPES` (all 20
...
- [ ] `AuditView` restructured: shared filter toolbar above antd
...
- [ ] **Export CSV** in the toolbar: Blob fetch of
...
- [ ] Error toasts (403/502/503) on both tabs and the export;
      busy states; role gate unchanged
- [ ] Vitest: tab switch preserves filters, summary fixture
...
- [ ] `authorization-matrix.md` (reporting rides the existing
      `audit:read` grant), `portal-user-guide.md` (Audit tabs +
      export), `configuration-reference.md`
      (`AUDIT_EXPORT_MAX_ROWS`)
- [ ] Version lockstep 0.27.6 → 0.28.0; `make verify` green before
      **and** after `make build`; `make deploy`
- [ ] Browser live check: filtered summary, export download +
...
- [ ] CHANGELOG 0.28.0 + release note + index; commit/scan/tag/push
