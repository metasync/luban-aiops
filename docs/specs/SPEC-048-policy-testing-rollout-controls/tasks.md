...
- [ ] `platform-gateway/services/policy_engine.py`: `load_bundle`
...
- [ ] `shared/shared-contracts/policies/policy-scenarios.yaml`: two
...
- [ ] Root `Makefile`: `validate-policy-scenarios` target (run under
      both products' uv envs, the `validate-policy` pattern); wired
      into `verify`
- [ ] Self-test: mutated fixture bundle → non-zero; identical → zero

## R-3: `policy-diff` impact report (W-3)

- [ ] `shared/shared-contracts/scripts/policy_diff.py`: per-(role,
...
- [ ] Root `Makefile`: `policy-diff` target with mandatory
      `CANDIDATE` (usage error on absence), run under both products'
      uv envs
- [ ] Tests: each transition category on fixture pairs, hard-error
      paths, identical no-op

## R-4: Rollout runbook (W-5)

- [ ] `docs/guides/configuration-reference.md`: policy rollout
...
- [ ] Both products' `test_policy_engine.py` parity assertions gain
...
- [ ] Version lockstep 0.29.3 → 0.30.0; `make verify` green before
      **and** after `make build`; `make deploy`
- [ ] Live check: provenance hash on the matrix surface matches the
      ...
      regression
- [ ] CHANGELOG 0.30.0 + release note + index; commit/scan/tag/push
