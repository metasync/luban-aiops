...
- [x] `make verify` green (all product pytest; kustomize overlays; policy rules + the new `session:skill_graduate` scenario; api + tools scenarios; version lockstep; the secret-vocabulary leg — extended if `KNOWN_SAFE_FIELDS` needs a twin check)
  - Green at 0.36.0: `VERIFY_EXIT=0`, **2424** python tests across the eight products (agent-platform 1142, platform-gateway 354, tool-gateway 336, skills-hub 184, audit-service 138, incident-service 137, execution-runtime 73, identity-broker 60), four kustomize overlays (`dev-k8s` + the three runtime profiles), **18** policy rules, **137** api + **19** tools scenarios with every granted pair covered (90 api / 13 tools), `OK: all product and portal versions match VERSION=0.36.0`.
  - The secret-vocabulary leg **was** extended, as this box anticipated: it now reports three agreements rather than one — the agent-platform/tool-gateway secret-parameter tuples (20 substrings), the agent-platform/skills-hub credential-hole markers (`<credential-reference>`, the stage-5 drift guard so R-2's placeholder and R-4's refusal check cannot diverge), and the agent-platform/tool-gateway secret-shape vocabularies (4 patterns, in order).
- [x] portal `npm test` and `npm run build` green (`tsc --noEmit` + vite)
  - **342** tests across **29** files green and `npm run build` clean (`BUILD EXIT=0`; the >500 kB chunk-size advisory is pre-existing and non-fatal).
  - Landing `npm test` green took **one test-harness fix**, recorded here because a passing suite was exiting non-zero: React 19's scheduler defers root work to a macrotask (`setImmediate` under jsdom, there being no MessageChannel) and antd's rc-motion advances an animated dialog's enter step over several animation frames, so a test that opens one — the shared skill-draft preview, any `Modal`, an imperative toast — can leave a scheduler task queued when it finishes. Vitest destroys the file's jsdom environment on worker hand-off, the task then fires against a deleted `window` (`ReferenceError: window is not defined`, from `performWorkOnRootViaSchedulerTask` reading `window.event`), and vitest reports it as an uncaught exception **attributed to whichever file the worker runs next** — so every test passed, the suite still exited `1`, and the blame landed on files this spec never touched (`ModelSelect`, `DraftAsSkillButton`, `IncidentsView`). Proven rather than assumed: 10/10 clean runs of the same suite at the SPEC-054 delivery commit in a throwaway worktree, and the leak isolated to the new file by exclusion (2/2 clean with it excluded, 2/2 dirty with it). `src/test/setup.ts` now drains the scheduled work after **each** test, while the environment is alive and `act` is still legal, and scopes `IS_REACT_ACT_ENVIRONMENT` to that drain — two wrong turns are worth recording: an `afterAll` (per-file) drain made it *worse* (4/5 dirty) because the task is queued per test and flushing React work outside a test prints a wall of "not wrapped in act" warnings, and leaving the act flag set globally turned those warnings on for every async mock that resolves after a test's `act` block (54–75 warnings against a baseline of **0**, in tests that are correct as written). With the fix: **13/13** consecutive full-suite runs `EXIT=0`, zero uncaught exceptions, zero act warnings, duration unchanged (~20–25s). No product behavior is involved and `make verify` does not run the portal suite, which is why this is recorded here and in the release note rather than in `CHANGELOG.md`.
  - The gate review accepted the fix's shape — the `finally` restore is right, hook ordering is safe (vitest runs each file's own `afterEach`, including the two fake-timer files' `vi.useRealTimers()`, *before* this drain, so it never hangs on faked timers), it fixes a jsdom-teardown race rather than papering over a product leak (`window` is never deleted in production), and shared setup is the right home because any `Modal`/`Drawer`/toast can trigger it, not just the new test — but flagged the **fixed frame count** as fragile: a future longer transition (a nested modal, a multi-step enter) could leave work queued past it and silently reintroduce the same misattributed non-zero exit. Hardened rather than deferred. The drain now **interleaves** a frame and an immediate per iteration instead of batching all frames then a single immediate, because work drained by the immediate — a React commit — can itself queue the next frame, which a trailing immediate would leave behind; and the constant is documented as a **ceiling** rather than a tuned value, naming the symptom that means it was exceeded so the next reader raises it here instead of adding a per-file drain. Re-measured after the change: **5/5** further full-suite runs `EXIT=0`, 342 green, zero uncaught, zero act warnings, duration 20.2–25.5s against the 21.7–30.8s band measured immediately before it (no regression), and `npm run build` still `EXIT=0` with the portal diff held to this one file (+53/-1).
- [x] version lockstep bumped **0.35.0 → 0.36.0** (MINOR): `VERSION` + every `pyproject.toml` + `metadata.py` + `__init__.py` literal + per-product `uv.lock` re-locks; `make validate-version` reports OK
  - 19 literal surfaces (`VERSION`, 8 `pyproject.toml`, 8 `metadata.py`, and the 2 `__init__.py` that carry one — audit-service and incident-service) plus 8 `uv.lock` re-locks at exactly one line each, without which `uv sync --frozen` fails. `make validate-version` → `OK: all product and portal versions match VERSION=0.36.0`. The portal's `vite.config.ts` reads `VERSION` at build time rather than carrying a literal, and `package.json` is `0.0.0` and is not a lockstep surface. The only surviving `0.35.0` in tracked source is the deliberate migration comment in `authoring_trace.py`.
- [x] `make build` produces the clean image; `make deploy` rolls it onto dev-k8s
  - `BUILD_EXIT=0` at `IMAGE_TAG=0.36.0-dev-k8s-6c45e21` — a **clean** tag with no `-dirty`, all nine images built (`agent-service`, `audit-service`, `execution-runtime`, `identity-service`, `incident-service`, `platform-gateway`, `skills-hub`, `tool-gateway`, `web-ui`). Reaching a clean tag cost one `chore:` commit (`6c45e21`): the two `.sqlcheck-spec055-*.sql` files this spec's own boxes cite as evidence were untracked and unignored, and the Makefile derives `-dirty-<timestamp>` from a non-empty `git status --porcelain`. Deleting them was the alternative, but this file names `.sqlcheck-spec055-s6a.sql` three times as the artifact proving R-4's migration against a real Postgres 16, so the references would have dangled in delivered docs; `.gitignore` gained a `.sqlcheck-*.sql` pattern instead, for the reason its `.*.log` entry already spells out.
  - `DEPLOY_EXIT=0`, then all twelve deployments `1/1 READY` with the nine luban images rolled from `0.35.0-dev-k8s-a30354d` to `0.36.0-dev-k8s-6c45e21`, the session store provisioned and the realm plus portal OIDC client reconciled. `make deploy-samples` re-installed the sample skills and restarted skills-hub to re-ingest (`SAMPLES_EXIT=0`) — the `skills-samples` ConfigMap act 3 merges the graduated draft into. An L3 deep security review of the delivery ran before the build and returned **zero findings**.
- [x] living state docs updated (see spec `Impact`): `CHANGELOG.md` (a `## 0.36.0` section), a dated release note + the notes README index, `docs/guides/configuration-reference.md` (**three** knobs, not the two this box anticipated), `docs/agentic-aiops-platform/authorization-matrix.md` (the new `session:skill_graduate` action), affected RepoWiki pages
  - `CHANGELOG.md` gained a `## 0.36.0 — 2026-09-09` section (six `### Added`, three `### Changed`, four `### Fixed`), and the release note is `docs/agentic-aiops-platform/release-notes/2026-09-09-develop-as-you-go-skill-graduation.md` with a matching index bullet in that directory's `README.md`. Both were written from the stage boxes, the refinement notes and the real route paths rather than from recall.
  - The knob count in this box was **stale when written**: there are three SPEC-055 knobs — `AGENT_AUTHORING_TRACE_MAX_STEPS` (`100`), `AGENT_AUTHORING_TRACE_IDLE_DAYS` (`180`, `0` disables the idle-GC) and `AGENT_SKILL_GRADUATION_MAX_STEPS` (`20`, a deliberate twin of the tool-gateway's `GATEWAY_BROWSER_FLOW_MAX_STEPS` replay budget). All three rows landed during **stage 6b** rather than at this gate: the two trace knobs had been missing from the reference since R-1, and the third arrived with the blast-radius bound — which is where the overclaim was found, since R-4's refusal reads a knob no doc surface named.
  - RepoWiki is committed separately per convention (never mixed into a feature commit). Two hand-settles are recorded rather than passed off as generated: the refresh snapshot predates this gate's `delivered` flip, so the SPEC-055 page's Introduction and Update Summary state the v0.36.0 delivery to match its SPEC-054 sibling; and three knowledge pages carried a stale "currently `0.35.0`" root-`VERSION` literal. A fourth page's illustrative `e.g. 0.35.0` was deliberately left alone — that one is an illustration, not a claim about the repo.
- [x] `docs/specs/README.md` SPEC-055 row → `delivered`
- [x] `docs/adr/README.md` ADR-0009 stays `accepted` (implemented by this delivery)
  - Verified rather than edited: the index row already reads `accepted`, which is the correct end state for an ADR this delivery implements — an ADR's status records the decision, not the shipping state of the code that realizes it.
- [x] `docs/agentic-aiops-platform/delivery-roadmap.md` SPEC-055 backlog row → `delivered` with the shipping version; the deferred OQ-2 infra-binding slice anchored to its own follow-up train
  - The row now reads "Delivered 2026-09-09 (0.36.0)" with the shipping evidence, and states explicitly that **OQ-2 did not ship in this train**. Rather than let that deferral live only in a spec footnote, it gets its own backlog row — "Generalized non-browser (infra) executable-flow binding" — naming the fallback that ships instead (an infra executable flow's steps park per-action under SPEC-054 R-2, so N infra writes cost N approvals where a browser flow costs one: a usability gap, not a safety one) and a promote-on signal (the first infra executable flow an operator actually graduates and replays).
- [x] spec.md Status block → `delivered`, release slice fixed to the shipping train, delivery changelog entry appended
  - `status: delivered`, `delivered: 2026-09-09 (v0.36.0)`, the release slice pinned to "seventeenth R5 slice, v0.36.0", ADR-0009 annotated as **implemented by this delivery** while staying `accepted`, and the sequencing line rewritten to past tense now that A→B→C is complete — plus a new `- deferred follow-up:` bullet carrying OQ-2. The appended Changelog entry records the delivery requirement-by-requirement and the two structural guarantees worth restating: a flow's `steps` list is never an input to its own gate (R-5), and a step with no observed origin is *unverified* rather than drifted, so it refuses (R-4).
- [x] browser live check on the canonical dev-k8s deployment: a session of approved mutations captures an ordered trace; graduating it produces a validated executable-flow draft (not published); a merged+ingested browser executable flow replays under **one** gate with every write signed + receipted; an action card shows no plaintext secret at rest, on the stream, or in the expander
  - Green at `0.36.0-dev-k8s-6c45e21` (`.livecheck-spec055-gate-chat3.log`): all six deterministic legs plus all four chat acts, ending on the contrast the sample exists to show — *authored ad hoc → 2 per-action card(s) for 2 mutation(s)*, *replayed as a graduated flow → 1 card for the same work*. Act 1 parked 2 per-action cards and the durable detail held 2 succeeded, signed write-tier executions; act 2 graduated them into `batch-password-reset-graduation-demo.md` with `2 step(s)`, `web_target http://browser-check-target:8080/admin/` and `declaration preceded` — "no model call, no invented step", and nothing published; act 3 merged into `skills-samples` and read back `kind=executable_flow risk_class=write steps=2` from the real store; act 4 parked exactly one `flow`-kind card, the resumed replay parked no second gate, and **8** write-tier executions each succeeded and carried a signed receipt under that single card. The 8 is not a deviation: act 4 asserts the one-gate invariant tool-agnostically rather than pinning an execution count, because a model may batch or repeat, and 8 sits inside the 20-step replay budget.
  - The fourth clause needed a **separate probe**, because this demo parks no secret-bearing card at all — by design it routes the new password through a read-tier `web.navigate` URL parameter and credentials through read-tier `web.fill_credential`, so there is nothing on its cards to mask. The probe forced the residual case `OPAQUE_VALUE_FIELDS` exists for (a literal typed directly into a field with `web.type`) and asserted all three legs: **stream** — the literal absent from the whole `confirmation_request` frame, `parameters.text == '***'`, `change_request` projection clean; **at rest** — the literal absent from every persisted confirmation card, `parameters == {'ref': '***', 'text': '***'}` (`ref` masks too, not being on `KNOWN_SAFE_FIELDS`, which is R-7's fail-closed posture rather than an oversight), surviving only in the operator's own transcript message — asserted to be a `user` entry and absent from `evidence_turns`, so the exclusion cannot silently widen; **expander** — a browser check on the canonical portal origin rendered `{"ref": "***", "text": "***"}` in the expanded *Technical details*, the collapsed card showing `text ***` with a `masked` badge, and a DOM containment check on the card region returned `false` for the literal.
  - The live check found **three sample defects, all fixed here, none in the product** — which is why no rebuild or redeploy was needed: this touches only `samples/`, and neither `demo.sh` nor the walkthrough ships inside an image. (1) *Act 1's first run failed* on its own prompt: it asked the model to click "Sign in", but the target's login page carries a legacy-SSO auto-login timer that hides the form and navigates away within 100 ms of both credential fields holding a value, so the click hit a detached element (`ElementHandle.click: Element is not attached to the DOM`). The model recovered and completed both resets, but the failed write stayed in the trace with **no observed origin**, and act 2 would refuse the whole session over it — R-4 behaving correctly, since fabricating an origin would corroborate a mutation that never landed. Both prompts and their two `WALKTHROUGH.md` mirrors were rewritten to wait for the redirect instead; the two sibling demos already leave that click out, so this is catching up to a posture the repo holds, not inventing one. (2) *Act 3's first re-run failed* on its one un-retried call: the skill-detail GET answered `502 {"detail":"skills hub unavailable"}`, the gateway's mapping of an httpx transport error, so nothing reached skills-hub. Reproduced **3 runs out of 3** in a controlled probe (restart → `rollout status` → LIST `200` → DETAIL `502` → DETAIL `200`), i.e. the Service endpoints are still propagating when `rollout status` returns. The demo already bounded-retried the inventory call for an analogous reason (readiness does not gate on ingestion) but made this call exactly once, against a service the act itself had just restarted; it now retries on `502`/`503` only, so a `404`/`401`/`403` — a real answer about that skill — still fails at once. The retry did not need to fire on the green run. (3) *Step 1 of the walkthrough was unreachable as written*: it told the reader to open the portal on a `svc/web-ui` localhost port-forward and sign in there, but the identity-broker starts every login at `OIDC_REDIRECT_URI`, and `shared/platform-ops/gitops/dev-k8s/README.md` states the canonical origin is the only one where sign-in round-trips. Step 1 now names that origin and says why a localhost tab stays signed out, with a matching troubleshooting row. **The two sibling web-check walkthroughs carry the same stale instruction**; they are out of this spec's scope, so they are flagged rather than fixed here.
  - One probe defect is worth recording because it is a lesson about the check rather than the platform: the first at-rest assertion tested the whole session record and failed, but the only occurrence was `.transcript[0].content` — the probe's own prompt, which has to spell the literal out for the model to type it. Masking the conversation record is not R-7's claim, so the assertion was scoped to the card subtree with the transcript exclusion asserted instead of assumed. The card was correctly masked on that first run too; the failure was the probe's, not the platform's.
