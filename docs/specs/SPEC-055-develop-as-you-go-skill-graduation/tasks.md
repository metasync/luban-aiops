...
- [ ] `services/authoring_trace.py` (new): `AuthoringTraceStore` `Protocol` + `InMemoryAuthoringTraceStore` + `PostgresAuthoringTraceStore` + `build_authoring_trace_store()` factory + module-level `AUTHORING_TRACE_STORE` singleton, mirroring `execution_records.py` (reuse `AGENT_STATE_STORE_BACKEND` / `AGENT_STATE_DB_URL`, fail-open-to-memory) (R-1)
- [ ] trace step shape: `session_id`, `position` (per-session ordinal), `tool_name` (canonical dotted), `args` (JSONB, secret-safe parameterized), `execution_id` + `confirm_id` (**references**, not the receipt), `status`, `captured_at` — every field on **both** backends (R-1)
- [ ] `authoring_trace` table: idempotent DDL created in place on first use, sharing the SPEC-016 `sessions` database; ordered by `(session_id, position)` (R-1)
- [ ] lifecycle `draft → graduated | discarded` (terminal); **not** time-swept with the 30-day `execution_records` window (R-1 / OQ-1)
- [ ] per-session step cap `AGENT_AUTHORING_TRACE_MAX_STEPS` (default 100) enforced at append — capture beyond the cap drops best-effort (R-1)
- [ ] idle-GC `AGENT_AUTHORING_TRACE_IDLE_DAYS` (default 180) reclaims `draft` traces idle beyond the window at startup + opportunistically on write; terminal rows are never swept (R-1 / OQ-1)
- [ ] `core/config.py` / `runtime_settings.py`: register the two knobs with defaults (R-1)
- [ ] tests: dual-backend round-trip incl. the `postgres` backend — every field survives on both (R-1)
- [ ] tests: the per-session step cap drops capture beyond the bound (R-1)
- [ ] tests: lifecycle transitions `draft → graduated` and `draft → discarded`; the idle-GC reclaims a `draft` row and never a terminal row (R-1)
- [ ] tests: a store failure degrades without raising (best-effort) (R-1)
