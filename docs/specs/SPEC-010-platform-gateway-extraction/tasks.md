...
- [ ] scaffold `products/platform-gateway/`: `pyproject.toml` (script
  `platform-gateway = platform_gateway.main:run`), `Makefile`
  (IMAGE_NAME `platform-gateway`), `Dockerfile`, `.python-version`
- [ ] move edge modules into `src/platform_gateway/`: core
...
- [ ] rename edge config keys to `PLATFORM_GATEWAY_*`
...
- [ ] move the edge half of the gateway test suite (app/routes, policy,
...
- [ ] dev-k8s: `base/tool-gateway/` becomes the tool service
...
- [ ] dev-k8s: broker `IDENTITY_TOKEN_AUDIENCE=platform-gateway` in
...
- [ ] `deploy-overlay.sh` sets images/rollouts for `platform-gateway` and
  `tool-gateway` (drops `api-gateway`); portal `nginx.conf` proxies to
  `platform-gateway:8000`
- [ ] `.github/CODEOWNERS` gains `/products/platform-gateway/`
- [ ] `make verify` green (all suites + all overlays render)

## R-5: Living-state docs advanced

- [ ] new `products/platform-gateway/README.md`; rewritten
  `products/tool-gateway/README.md`; identity-broker README wording
- [ ] workspace docs: `workspace-model.md`, `product-boundaries.md`,
  `backend-service-layout-convention.md`, `github-repository-governance.md`
  gain/adjust the `platform-gateway` product
- [ ] dev-k8s README: service list, wiring, secrets, SPEC-009 sections
  updated for the two services; one-time `api-gateway` cleanup command
  documented
- [ ] root `README.md` product list and current state; identity design
  doc service list

## Delivery Gate

- [ ] all acceptance criteria in `spec.md` verified (`make verify` green)
- [ ] `CHANGELOG.md` entry added referencing SPEC-010
