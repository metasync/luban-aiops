...
- 2026-07-30: delivered — all requirements (R-1…R-7) implemented and validated; `make verify` green (agent-platform 88, identity-broker 38, tool-gateway 105, all overlays render). SPEC-007 R-4/R-6 closed and Q-1/Q-2 resolved; SPEC-007 advanced to `delivered`. Status set to `delivered` and spec frozen.
- 2026-07-30: the R-3 workload-identity upgrade path is implemented by SPEC-009 — the exchange now accepts projected service-account tokens (`IDENTITY_WORKLOAD_ISSUER_URL` / `IDENTITY_WORKLOAD_AUDIENCE` / `IDENTITY_WORKLOAD_CLIENTS`) and the gateway prefers `GATEWAY_WORKLOAD_TOKEN_PATH` with the static secret as the documented dev fallback
