...
- [ ] dev-k8s wiring: agent-platform credential in the incident-service
      query-auth registry Secret + URL knob via the existing
      Secret-sync conventions
- [ ] Tests: not-configured 503, unauthorized 401, unknown-id 404,
      transport-failure 502, both gate denials

## R-4: Prose layer (inherited, digest-only)

- [ ] Extend the prose prompt builder for the incident-report digest;
      prompt-purity regression test (digest fields only)
- [ ] `prose_status` behavior and rendering identical to shift
      summaries

## R-5: Audit (no new event types)

- [ ] `document_created` carries `incident_id` for the new type (agent
      layer + gateway log event); published/cross-owner-read events
      unchanged
- [ ] Tests asserting event payloads and the no-new-event-type
      invariant

## R-6: Portal Documents support

- [ ] Creation dialog: type radio, incident picker (existing incidents
      list surface), label + prose toggle retained
- [ ] Drawer rendering: Incident / Triage / Dispatches / Session tabs
      + Raw JSON, narrative panel, marker alerts, foreign metadata
      banner; type badge on list rows; envelope-only listings intact
- [ ] Vitest coverage for dialog + rendering; zero antd deprecation
      warnings (guard active)

## Delivery train

- [ ] Living-state docs: portal-user-guide, incident-guide,
      authorization-matrix, documents-digest-reference,
      configuration-reference
- [ ] Version lockstep to 0.25.0, CHANGELOG + release note + index
- [ ] make build → make verify → make deploy → live checks (triaged
      incident report create/publish/cross-owner read on the durable
      trail; `triage_failed` and session-less marker paths)
- [ ] Commit, security scan gate, tag v0.25.0, push, repowiki docs
      commit
