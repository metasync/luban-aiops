...
Document lists show a one-line **summary** under each label, e.g.
*2 sessions · 3 decisions · 1 execution · 1 open item* or *Quiet shift
— no recorded decisions or executions.* The summary is computed
deterministically at creation time from the handover skeleton and is
**counts-only**: it never contains session titles, record ids, decision
outcomes, or narrative text. Documents created before summaries
existed show label-only rows — that is expected, not an error.
...
digest alone**, after the digest is frozen. It is always labeled
*AI-generated narrative — from this document's digest facts*; when
generation fails, a warning is shown and the digest stands alone. The
narrative is a reading aid — the digest remains the artifact of record,
and only the digest is guaranteed deterministic.

### Envelope-only lists

Document lists are deliberately **envelope-only**: they return ids,
labels, state, owner, timestamps, and the counts-only summary — never
the digest or the prose. Full content is served only by the single
document fetch, which is the surface that carries the cross-owner read
audit. Browsing a list is cheap and un-audited; opening someone else's
document is one audited action.
