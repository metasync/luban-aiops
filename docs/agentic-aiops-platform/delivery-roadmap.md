...
| Portal framework rebuild | Rebuild the operator portal on an AI UI framework with built-in multi-session and voice components; consumes SPEC-022's session API and satisfies its Appendix A UI contract. Spike memo landed 2026-08-22 recommending Ant Design X on React with a platform-owned SSE contract adapter; promoted 2026-08-22 to `SPEC-023-portal-framework-rebuild` (`draft`, 0.9.0 train). | `docs/specs/SPEC-023-portal-framework-rebuild/` |
| Runtime LLM model switching | Portal dropdown to choose the model per session (default deepseek) instead of deploy-time runtime profiles. Requires multi-provider settings/secrets in agent-platform, a request-level provider selection, per-session affinity, and audit of the chosen model; dropdown UI lands with the SPEC-023 rebuild (R-1 leaves the composer room for the selector). | own spec (SPEC-024 candidate) |
