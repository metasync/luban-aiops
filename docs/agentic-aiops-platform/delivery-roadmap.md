...
| Skill authoring export from sessions | Raised in the 2026-08-27 operator review beside the document repository: turn a session's troubleshooting/triage into skill markdown that the operator contributes to their own team's Git skills repo (already ingestible through skills-hub's federated sources, SPEC-014 lineage). Deliberately not a document type — the artifact of record stays in Git-managed team knowledge; the authoring experience (session → validated markdown draft, digest-only generation posture) is the feature. Spiked 2026-08-29 in `docs/workspace/skill-authoring-spike.md`: recommended shape is an ephemeral draft (agent-platform generates from the session's durable facts in the `generate_prose` posture, the draft is validated against Skill Format v1 before it reaches the operator, and the export is a client-side `.md` download — the platform drafts, humans merge into their team Git repo; no platform write path into skills sources), with five spec-sized open questions (input scope, validation hosting, content guardrails, policy action/audit, provenance marker); promote to a spec on operator sign-off of the recommendation. | skills-hub enhancement spec |
