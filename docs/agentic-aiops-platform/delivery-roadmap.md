...
| Portal framework rebuild | Rebuild the operator portal on an AI UI framework (Ant Design X, AgentScope Spark, or similar) with built-in multi-session and voice components; consumes SPEC-022's session API and satisfies its Appendix A UI contract, including the model dropdown UI. Spike memo landed 2026-08-22 recommending Ant Design X on React with a platform-owned SSE contract adapter (`docs/workspace/portal-framework-rebuild-spike.md`); promotion to SPEC-023 pending review. | own spec (SPEC-023 candidate) after SPEC-022 |
