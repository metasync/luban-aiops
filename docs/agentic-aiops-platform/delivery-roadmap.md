...
| Develop-as-you-go skill graduation | Approved 2026-09-07 as `SPEC-055-develop-as-you-go-skill-graduation` (seventeenth R5 slice), the graduation destination of the A→B→C program, implementing **ADR-0009** (accepted 2026-09-07): a durable dual-backend authoring-trace store separate from the 30-day `execution_records` receipt sweep, capturing secret-safe parameterized steps as a by-product of each already-approved already-signed mutation; an executable-flow skill class declaring `risk_class: write` **decoupled from `web_target`** with a machine-readable replay step list; deterministic graduation with blast-radius re-validation producing a draft for human merge (never auto-publish) behind one new `session:skill_graduate` action and one new `skill_graduated` audit event; and one-gate secret-safe replay with credentials resolved from credential-set references. Open questions resolved at approval on the draft's own recorded recommendations, with OQ-2 scoped so browser replay ships in this spec while the generalized non-browser (infra) flow binding is targeted at 0.36.0 — infra steps meanwhile park per-action under SPEC-054, which fails safe; and **R-7** (folded post-approval from the SPEC-054 delivery review) hardens the change-request card's secret masking at the approval seam — fail-closed projection, no plaintext secret on the stream/render surface, signed `args_digest` preserved — the source-side complement of R-2's secret-safe capture. **Sequenced after SPEC-054** (now `delivered`, so R-7 folds in before planning); its `plan.md` is authored once B is `delivered`. | `docs/specs/SPEC-055-develop-as-you-go-skill-graduation/` |
