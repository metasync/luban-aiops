...
| Runtime LLM model switching | Portal dropdown to choose the model per session (default deepseek) instead of deploy-time runtime profiles. Requires multi-provider settings/secrets in agent-platform, a request-level provider selection, per-session affinity, and audit of the chosen model; the SPEC-023 rebuild delivered the React/antd composer shell the selector lands in. | own spec (SPEC-024 candidate) |
