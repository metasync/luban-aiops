...
| Portal dependency hygiene | Raised in the v0.23.2 delivery train: the vitest suite prints 53 antd deprecation warnings (48× Drawer `width`, 5× Alert `message`) and the same-day upgrade check found the major components several majors behind upstream (vite 6 vs 8, vitest 3 vs 4, TypeScript 5.6 vs 5.9, React 18 vs 19, jsdom 25 vs 30). Drafted 2026-08-28 as `SPEC-042-portal-dependency-hygiene` directly from the check (memo-free, SPEC-031/032 departure): deprecation migration plus a zero-tolerance vitest guard, the recorded adopt set, and the React 19 migration with a behavioral gate; TypeScript 7.x parked as too new. | `docs/specs/SPEC-042-portal-dependency-hygiene/` |
