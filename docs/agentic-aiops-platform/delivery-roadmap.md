...
| Studio — a dedicated skill-development workspace | Drafted 2026-09-12 memo-free from the 2026-09-08→12 operator design discussion (the SPEC-045/046/049 precedent) as `SPEC-056-studio-skill-development-workspace` (eighteenth R5 slice, targeting v0.37.0): split the single operator-portal **Chat** entry into **Chat** (operation sessions) and a new **Studio** (development sessions) over **one shared `ChatView` core** parameterized by a `mode`, backed by an additive two-value `session_type` discriminator (operation/development) fixed at birth and promoted one way. Delivers the operator's four goals — two distinct entries; a clean role mapping (**Option A**: Studio is an authoring power held by operator/approver/platform-admin, developer/read-only-observer/auditor keep Chat only); correct document generation (the shift-summary picker filters to operation sessions, closing a live unfiltered-picker defect in `DocumentsView.tsx`); and blast-radius control (the SSE stream, secret masking, and HITL core are shared and asserted identical across modes, never forked). **Design B** control placement: "Draft as skill" stays in Chat (operational knowledge export), "Declare a target" + "Graduate as skill" move to Studio, and a one-way "Move to Studio" promotion — the existing `session:skill_graduate`-gated declare-target route extended to flip `session_type`, the single writer — is the bridge (the authoring trace is captured regardless of target per SPEC-055 R-2, so a promoted session graduates exactly as a born-development one). No new policy action (development-session creation dual-gates on the existing `session:skill_graduate`), no new audit event type, no change to the graduation/replay trust model. **Status `draft`** — OQ-1..OQ-5 (entry-gating mechanism, legacy backfill, promotion mechanics, list scoping, flip audit) each carry a recommendation for resolution at approval. The companion role-vocabulary reconciliation (retire the design-only `senior-operator`, document `developer`) landed separately as a docs-only patch on 0.36.3. | `docs/specs/SPEC-056-studio-skill-development-workspace/` |
| Generalized non-browser (infra) executable-flow binding | SPEC-055's **OQ-2**, deferred at approval and re-anchored here at that delivery rather than shipped: today's flow binding (`FlowContext`/`web.navigate(skill_id=…)`) is browser-specific, so an infra (`k8s.*`) executable-flow skill — now authorable and ingestable since SPEC-055 R-3 decoupled `risk_class: write` from `web_target` — has no binding analog, and its steps park **per-action** under SPEC-054 R-2 instead of collapsing to one gate. That fallback fails safe (each write still approved, signed, audited, receipted, and joining no auto-allow list) and is asserted by SPEC-055 R-5's tests, so this is a usability gap rather than a safety one: N infra writes cost N approvals where a browser flow costs one. Promote on the first infra executable flow an operator actually graduates and replays, since the shape of the binding should be driven by a real `k8s.*` step list rather than a speculative generalization of the browser one — the browser gate keys on origin allowlist plus path containment, and an infra flow needs an equivalently concrete scope (resource/namespace/verb) before a step budget means anything. | agent-platform kernel + tool-gateway flow authority |
