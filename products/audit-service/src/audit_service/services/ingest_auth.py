"""Ingest/query caller authentication (SPEC-013 R-3, Q-3 refinement).

Reuses the SPEC-008/009 credential vocabulary, verified locally — no runtime
dependency on the identity broker:

- static path: HTTP Basic against the ``AUDIT_INGEST_CLIENTS`` registry
  (mirrors identity-broker ``authenticate_client``)
- workload path: Kubernetes projected service-account token presented as a
  ...
  and subject-registry checks (mirrors identity-broker
...
def authenticate_static(
    settings: AuditSettings, client_id: str | None, client_secret: str | None
) -> str:
    """Validate an HTTP Basic credential against the registry."""
    if not client_id or not client_secret:
        raise IngestAuthError("service credential required")
    for client in settings.ingest_clients:
        if client.client_id == client_id and client.secret == client_secret:
            return client.client_id
    raise IngestAuthError("invalid service credential")
...
def _get_workload_jwks_client(settings: AuditSettings) -> jwt.PyJWKClient:
...
def authenticate_workload(settings: AuditSettings, bearer_token: str) -> str:
    """Validate a projected workload token and map it to a registered client."""
    if not settings.workload_issuer_url:
        raise IngestAuthError("workload identity not enabled")
    try:
        ...
        raise IngestAuthError("workload token expired") from exc
    except jwt.InvalidTokenError as exc:
        raise IngestAuthError("workload token invalid") from exc
    subject = str(claims.get("sub", ""))
    for mapping in settings.workload_clients:
        if mapping.workload_subject == subject:
            return mapping.client_id
    raise IngestAuthError("workload subject not registered")
...
async def authenticate_caller(settings: AuditSettings, request: Request) -> str:
