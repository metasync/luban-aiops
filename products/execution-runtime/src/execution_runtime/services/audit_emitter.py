...
service can never degrade the execution path. An unset
``EXECUTION_AUDIT_SERVICE_URL`` keeps the historical log-only behavior
byte-for-byte.
"""
...
from execution_runtime.core.metrics import record_audit_emit
from execution_runtime.metadata import SERVICE_NAME
...
def emit_audit_event(settings: ExecutionSettings, event: dict) -> None:
...
def _deliver(settings: ExecutionSettings, event: dict) -> None:
