"""Audit emitter tests (SPEC-013 R-3).
...
from identity_service.services import audit_emitter
...
def _settings(**overrides) -> IdentitySettings:
    defaults = {
        ...
        "audit_client_id": "identity-broker",
        "audit_client_secret": "ib-secret",
    }
...
class BuildAuditEventTests(unittest.TestCase):
    def test_envelope_validates_against_contract(self) -> None:
        ...
        self.assertEqual(event["service"], "identity-service")
        self.assertEqual(event["request_id"], "req-1")

    def test_optional_fields_omitted_when_absent(self) -> None:
        event = audit_emitter.build_audit_event("token_exchange", "req-1", "deny")
    ...
    def test_missing_request_id_falls_back_to_unknown(self) -> None:
        event = audit_emitter.build_audit_event("token_exchange", None, "success")
...
class EmitAuditEventTests(unittest.TestCase):
    def test_noop_when_url_unset_spawns_no_thread(self) -> None:
        ...
        event = audit_emitter.build_audit_event("token_exchange", "req-1", "success")
...
class DeliverTests(unittest.TestCase):
    def test_deliver_success_records_ok(self) -> None:
        ...
        self.assertEqual(fake.calls[0]["auth"], ("identity-broker", "ib-secret"))

    def test_deliver_non_2xx_records_error(self) -> None:
        settings = _settings()
        event = audit_emitter.build_audit_event("token_exchange", "req-1", "success")
        fake = _FakeClient(response=_FakeResponse(403))
        with (
            patch.object(audit_emitter.httpx, "Client", return_value=fake),
            patch.object(audit_emitter, "record_audit_emit") as record,
        ):
