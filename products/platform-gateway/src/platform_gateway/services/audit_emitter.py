...
service can never degrade the gateway path. An unset
``PLATFORM_GATEWAY_AUDIT_SERVICE_URL`` keeps the historical log-only
...
from platform_gateway.core.metrics import record_audit_emit
from platform_gateway.metadata import SERVICE_NAME
...
def emit_audit_event(settings: PlatformGatewaySettings, event: dict) -> None:
...
def _deliver(settings: PlatformGatewaySettings, event: dict) -> None:
