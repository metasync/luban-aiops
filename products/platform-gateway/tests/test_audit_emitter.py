...
from platform_gateway.services import audit_emitter
...
class BuildAuditEventTests(unittest.TestCase):
    def test_envelope_validates_against_contract(self) -> None:
        event = audit_emitter.build_audit_event(
            ...
            "req-1",
            "allow",
            details={"action": "chat", "decision": "allow"},
            subject="user-1",
            username="alice",
            roles=["operator"],
            session_id="ses-1",
        ...
        self.assertEqual(event["service"], "platform-gateway")
        self.assertEqual(event["request_id"], "req-1")

    def test_optional_fields_omitted_when_absent(self) -> None:
        event = audit_emitter.build_audit_event("policy_decision", "req-1", "deny")
    ...
    def test_missing_request_id_falls_back_to_unknown(self) -> None:
        event = audit_emitter.build_audit_event("policy_decision", None, "allow")
...
class EmitAuditEventTests(unittest.TestCase):
    def test_noop_when_url_unset_spawns_no_thread(self) -> None:
        ...
        event = audit_emitter.build_audit_event("policy_decision", "req-1", "allow")
