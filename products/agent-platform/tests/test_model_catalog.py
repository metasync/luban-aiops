"""Model catalog tests (SPEC-024 R-1).

Exercises the credential-gated catalog derivation: per-provider env knobs,
AGENTSCOPE_* fallback for the active provider, drop-without-key, default
flagging, and the credential-free discovery view.
"""
...
def test_additional_provider_enabled_by_own_key(clean_env):
    clean_env.setenv("OPENAI_API_KEY", "sk-openai")
    entries = build_model_catalog(_settings())
    by_id = {e.id: e for e in entries}
    assert set(by_id) == {"deepseek", "openai"}
    openai = by_id["openai"]
    assert openai.api_key == "sk-openai"
    assert openai.model_name == "gpt-4o-mini"  # adapter default
    assert openai.base_url is None  # OpenAI adapter has no default base URL
...
def test_per_provider_knobs_override_adapter_defaults(clean_env):
    ...
    entries = build_model_catalog(_settings())
    openai = next(e for e in entries if e.id == "openai")
    assert openai.model_name == "gpt-5"
    assert openai.base_url == "https://proxy.internal/v1"
...
def test_per_provider_knobs_never_fall_back_to_agentscope_key(clean_env):
    ...
    entries = build_model_catalog(_settings())
    assert [e.id for e in entries] == ["deepseek"]
...
def test_public_view_never_exposes_credentials(clean_env):
    clean_env.setenv("OPENAI_API_KEY", "sk-openai")
    catalog = ModelCatalog(build_model_catalog(_settings()))
