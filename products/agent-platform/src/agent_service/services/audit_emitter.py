...
service can never degrade the agent path. An unset
``AGENT_AUDIT_SERVICE_URL`` keeps the historical log-only behavior
byte-for-byte.
"""
...
from agent_service.core.config import RuntimeSettings
from agent_service.core.metrics import record_audit_emit
from agent_service.metadata import SERVICE_NAME
...
def emit_audit_event(settings: RuntimeSettings, event: dict) -> None:
...
def _deliver(settings: RuntimeSettings, event: dict) -> None:
