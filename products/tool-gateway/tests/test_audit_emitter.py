...
from tool_gateway.services import audit_emitter
...
def _settings(**overrides) -> GatewaySettings:
    defaults = {
        ...
        "audit_client_id": "tool-gateway",
        "audit_client_secret": "tg-secret",
    }
...
class BuildAuditEventTests(unittest.TestCase):
    def test_envelope_validates_against_contract(self) -> None:
        ...
        self.assertEqual(event["service"], "tool-gateway")
        self.assertEqual(event["request_id"], "req-1")

    def test_optional_fields_omitted_when_absent(self) -> None:
        event = audit_emitter.build_audit_event("tool_invoked", "req-1", "deny")
    ...
    def test_missing_request_id_falls_back_to_unknown(self) -> None:
        event = audit_emitter.build_audit_event("tool_invoked", None, "success")
...
class EmitAuditEventTests(unittest.TestCase):
    def test_noop_when_url_unset_spawns_no_thread(self) -> None:
        ...
        event = audit_emitter.build_audit_event("tool_invoked", "req-1", "success")
...
class DeliverTests(unittest.TestCase):
    def test_deliver_success_records_ok(self) -> None:
        ...
        self.assertEqual(fake.calls[0]["auth"], ("tool-gateway", "tg-secret"))
