...
service can never degrade the tool path. An unset ``GATEWAY_AUDIT_SERVICE_URL``
...
from tool_gateway.core.metrics import record_audit_emit
from tool_gateway.metadata import SERVICE_NAME
...
def emit_audit_event(settings: GatewaySettings, event: dict) -> None:
...
def _deliver(settings: GatewaySettings, event: dict) -> None:
