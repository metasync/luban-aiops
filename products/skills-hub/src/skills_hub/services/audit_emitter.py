"""Fire-and-forget audit event emitter (SPEC-029 R-2, SPEC-013 pattern).
...
service can never degrade the query path. An unset ``SKILLS_AUDIT_SERVICE_URL``
keeps the historical log-only behavior byte-for-byte.
"""
...
from skills_hub.core.metrics import record_audit_emit
from skills_hub.metadata import SERVICE_NAME
...
def emit_audit_event(settings: SkillsSettings, event: dict) -> None:
...
def _deliver(settings: SkillsSettings, event: dict) -> None:
