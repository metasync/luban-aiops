"""Query caller authentication (SPEC-014 R-3).

Reuses the SPEC-008/009 credential vocabulary, verified locally — no runtime
dependency on the identity broker — but against a distinct query registry
from day one (deliberately avoiding the SPEC-013 shared ingest/query
credential limitation):

- static path: HTTP Basic against the ``SKILLS_QUERY_CLIENTS`` registry
...
def authenticate_static(
    settings: SkillsSettings, client_id: str | None, client_secret: str | None
) -> str:
    """Validate an HTTP Basic credential against the query registry."""
...
def _get_workload_jwks_client(settings: SkillsSettings) -> jwt.PyJWKClient:
...
def authenticate_workload(settings: SkillsSettings, bearer_token: str) -> str:
...
async def authenticate_caller(settings: SkillsSettings, request: Request) -> str:
