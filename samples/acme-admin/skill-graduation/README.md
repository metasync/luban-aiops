...
It is the third of three `acme-admin` browser samples that drive the *same*
stateful console, and the only one that ships no skill:

| Sample | Skill | Approval shape |
|---|---|---|
| [`acme-admin/password-reset`](../password-reset/) | hand-written **bound flow** (`web_target` declared) | 1 `flow` card for N writes |
| [`acme-admin/adhoc-password-reset`](../adhoc-password-reset/) | hand-written runbook, **no `web_target`** | N `action` cards for N writes |
| `acme-admin/skill-graduation` (this one) | **none** — the skill is the artifact the demo produces | N `action` cards to author, then 1 `flow` card to replay |
...
| Path | Purpose |
|---|---|
| `demo/demo.sh` | Standalone demo script: six deterministic legs (always run) plus four opt-in acts — author → graduate → merge → replay (`RUN_CHAT_LEG=true`) — with acts 1 and 4 proving both resets landed in the store |
...
> `SAMPLE=acme-admin/skill-graduation` (which refuses, correctly, because there
> is no `skill/` to pack).
...
- A running dev-k8s cluster with the `browser-dev` **and** `mutating-dev`
  ...
  neither the authoring nor the replay can complete)
...
- Port-forwards for the identity broker and the platform-gateway:
  ...
  Unlike the other two `acme-admin` browser samples, the gateway forward is
  **not** chat-leg-only: legs 5 and 6 declare targets and attempt graduations,
  so the deterministic run needs it too.

## How it works

The four acts run against one `acme-admin` console and two throwaway users
(`alice`, `bob`):
...
1. Copy this directory to `samples/acme-admin/<your-sample>/` and update
...
(the same target `acme-admin/password-reset` and the `adhoc-password-reset`
sibling use, plus the browser infra the SPEC-049 `browser-check-demo.sh` smoke
test exercises):
