...
# skills-hub, agent-service, and execution-runtime emit audit events to
# the audit-service, which authenticates ingest callers against a static
...
#   execution-runtime →  EXECUTION_AUDIT_CLIENT_SECRET
#
...
AUDIT_INGEST_CLIENTS=tool-gateway=${AUDIT_INGEST_SECRET},platform-gateway=${AUDIT_INGEST_SECRET},identity-broker=${AUDIT_INGEST_SECRET},incident-service=${AUDIT_INGEST_SECRET},skills-hub=${AUDIT_INGEST_SECRET},agent-service=${AUDIT_INGEST_SECRET},execution-runtime=${AUDIT_INGEST_SECRET}
EOF
...
SH_SECRET_FILE="$BASE_DIR/skills-hub/runtime-secrets.env"
upsert_env_line "$SH_SECRET_FILE" SKILLS_AUDIT_CLIENT_SECRET \
...
# execution-runtime (SPEC-038 R-3) emits execution_completed /
# execution_rejected events from the worker's receipt authorship.
ER_SECRET_FILE="$BASE_DIR/execution-runtime/runtime-secrets.env"
upsert_env_line "$ER_SECRET_FILE" EXECUTION_AUDIT_CLIENT_SECRET \
  "EXECUTION_AUDIT_CLIENT_SECRET=${AUDIT_INGEST_SECRET}"
sync_secret execution-runtime-runtime-secrets "$ER_SECRET_FILE"
